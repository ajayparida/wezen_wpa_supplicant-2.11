...
#ifdef CFG_INTERFACE 
static void wiphy_info_supported_iftypes_cfg(struct wiphy_info_data *info,
					struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_AP))	
		info->capa->flags |= WPA_DRIVER_FLAGS_AP;
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_MESH_POINT))	
		info->capa->flags |= WPA_DRIVER_FLAGS_MESH;
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_ADHOC))	
		info->capa->flags |= WPA_DRIVER_FLAGS_IBSS;
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_P2P_DEVICE))
		info->capa->flags |= WPA_DRIVER_FLAGS_DEDICATED_P2P_DEVICE;
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_P2P_GO))
		info->p2p_go_supported = 1;
	if (wiphy_info->interface_modes & BIT(NRF_WIFI_IFTYPE_P2P_CLIENT))
...
#ifdef CFG_INTERFACE
static int wiphy_info_iface_comb_process_cfg(struct wiphy_info_data *info,
                                     struct nrf_wifi_event_iface_combination *iface_comb)
{
	int i;
	int combination_has_p2p = 0, combination_has_mgd = 0;
	if (iface_comb->comb_valid & NRF_WIFI_EVENT_GET_WIPHY_VALID_RADAR_DETECT_WIDTHS)
		info->capa->flags |= WPA_DRIVER_FLAGS_RADAR;
	for (i = 0; i < 2; i++)
	{
		int ift = iface_comb->limits[i].nrf_wifi_types;
	...
	if (combination_has_p2p && combination_has_mgd) {
		unsigned int num_channels = iface_comb->nrf_wifi_num_different_channels;
...
static void wiphy_info_iface_comb_cfg(struct wiphy_info_data *info,
				      struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	int i;
	for (i = 0; i < wiphy_info->num_iface_com; i++)
		if (wiphy_info_iface_comb_process_cfg(info, &wiphy_info->iface_com[i]) > 0)
			break;
}
static void wiphy_info_supp_cmds_cfg(struct wiphy_info_data *info,
                                      struct nrf_wifi_event_get_wiphy *wiphy_info)
{
...
static void wiphy_info_cipher_suites_cfg(struct wiphy_info_data *info,
                                      struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	int i, num;
	num = wiphy_info->n_cipher_suites;
	for (i = 0; i < num; i++) {
		u32 c = wiphy_info->cipher_suites[i];
...
static void wiphy_info_max_roc_cfg(struct wpa_driver_capa *capa,
                                      struct nrf_wifi_event_get_wiphy *wiphy_info)
{
		capa->max_remain_on_chan = wiphy_info->max_remain_on_channel_duration;
}
static void wiphy_info_tdls_cfg(struct wpa_driver_capa *capa,
                               struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	wpa_printf(MSG_DEBUG, "nl80211: TDLS supported");
	if (wiphy_info->get_wiphy_flags & NRF_WIFI_EVENT_GET_WIPHY_SUPPORTS_TDLS)
		capa->flags |= WPA_DRIVER_FLAGS_TDLS_SUPPORT;
	if (wiphy_info->get_wiphy_flags & NRF_WIFI_EVENT_GET_WIPHY_TDLS_EXTERNAL_SETUP) {
...
#ifdef CFG_INTERFACE
static void wiphy_info_ext_feature_flags_cfg(struct wiphy_info_data *info,
				     struct wpa_driver_capa *capa,
					 struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	u8 *ext_features = wiphy_info->ext_features;
	int len = wiphy_info->ext_features_len;
...
#ifdef CFG_INTERFACE
static void wiphy_info_feature_flags_cfg(struct wiphy_info_data *info,
					 struct wpa_driver_capa *capa,
					 struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	if (wiphy_info->features & NL80211_FEATURE_SK_TX_STATUS)
		info->data_tx_status = 1;

	if (wiphy_info->features & NL80211_FEATURE_INACTIVITY_TIMER)
		capa->flags |= WPA_DRIVER_FLAGS_INACTIVITY_TIMER;

	if (wiphy_info->features & NL80211_FEATURE_SAE)
		capa->flags |= WPA_DRIVER_FLAGS_SAE;

	if (wiphy_info->features & NL80211_FEATURE_NEED_OBSS_SCAN)
		capa->flags |= WPA_DRIVER_FLAGS_OBSS_SCAN;

	if (wiphy_info->features & NL80211_FEATURE_AP_MODE_CHAN_WIDTH_CHANGE)
		capa->flags |= WPA_DRIVER_FLAGS_HT_2040_COEX;

	if (wiphy_info->features & NL80211_FEATURE_TDLS_CHANNEL_SWITCH) {
		wpa_printf(MSG_DEBUG, "nl80211: TDLS channel switch");
		capa->flags |= WPA_DRIVER_FLAGS_TDLS_CHANNEL_SWITCH;
	}

	if (wiphy_info->features & NL80211_FEATURE_P2P_GO_CTWIN)
		info->p2p_go_ctwindow_supported = 1;

	if (wiphy_info->features & NL80211_FEATURE_LOW_PRIORITY_SCAN)
		info->have_low_prio_scan = 1;

	if (wiphy_info->features & NL80211_FEATURE_SCAN_RANDOM_MAC_ADDR)
		info->mac_addr_rand_scan_supported = 1;

	if (wiphy_info->features & NL80211_FEATURE_SCHED_SCAN_RANDOM_MAC_ADDR)
		info->mac_addr_rand_sched_scan_supported = 1;
#if 0
	if (wiphy_info->features & NL80211_FEATURE_STATIC_SMPS)
		capa->smps_modes |= WPA_DRIVER_SMPS_MODE_STATIC;
	if (wiphy_info->features & NL80211_FEATURE_DYNAMIC_SMPS)
		capa->smps_modes |= WPA_DRIVER_SMPS_MODE_DYNAMIC;

#endif
	if (wiphy_info->features & NL80211_FEATURE_SUPPORTS_WMM_ADMISSION)
		info->wmm_ac_supported = 1;

	if (wiphy_info->features & NL80211_FEATURE_DS_PARAM_SET_IE_IN_PROBES)
		capa->rrm_flags |= WPA_DRIVER_FLAGS_DS_PARAM_SET_IE_IN_PROBES;

	if (wiphy_info->features & NL80211_FEATURE_WFA_TPC_IE_IN_PROBES)
		capa->rrm_flags |= WPA_DRIVER_FLAGS_WFA_TPC_IE_IN_PROBES;

	if (wiphy_info->features & NL80211_FEATURE_QUIET)
		capa->rrm_flags |= WPA_DRIVER_FLAGS_QUIET;

	if (wiphy_info->features & NL80211_FEATURE_TX_POWER_INSERTION)
		capa->rrm_flags |= WPA_DRIVER_FLAGS_TX_POWER_INSERTION;

	if (wiphy_info->features & NL80211_FEATURE_HT_IBSS)
		capa->flags |= WPA_DRIVER_FLAGS_HT_IBSS;

	if (wiphy_info->features & NL80211_FEATURE_FULL_AP_CLIENT_STATE)
...
static void wiphy_info_probe_resp_offload_cfg(struct wpa_driver_capa *capa,
					  struct nrf_wifi_event_get_wiphy *wiphy_info)
{
#if 0
	u32 protocols;
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_PROBE_RESP_OFFLOAD)
		protocols = wiphy_info->probe_resp_offload;
	...
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_PROBE_RESP_OFFLOAD)
		capa->probe_resp_offloads = wiphy_info->nrf_wifi_probe_resp_offload;
}
#else
...
#ifdef CFG_INTERFACE
static void wiphy_info_extended_capab_cfg(struct wpa_driver_nl80211_data *drv,
				      struct nrf_wifi_event_get_wiphy *wiphy_info)
{
	//TODO
	int i = 0;
	if (!wiphy_info->extended_capabilities_len ||
	    drv->num_iface_capa == NL80211_IFTYPE_MAX)
		return;

	do {
		unsigned int len;
		struct drv_nl80211_iface_capa *capa;
	/*TODO: Only for STA interface extended capability is added.
	 * For Other interfaces need to be Done . 
	 * f/w need to provide extended capabilites for other interfaces
	 */
		capa = &drv->iface_capa[drv->num_iface_capa];
	//	capa->iftype = wiphy_info->interface_modes;
		...
		capa->ext_capa = os_memdup(&wiphy_info->extended_capabilities[0],
					   len);
		if (!capa->ext_capa)
			goto err;
		...
		len = wiphy_info->extended_capabilities_len;
		capa->ext_capa_mask =
			os_memdup(&wiphy_info->extended_capabilities_mask[0],
				  len);
...
#ifdef CFG_INTERFACE
struct cfg_wiphy_info_arg {
        struct wpa_driver_nl80211_data *drv;
	struct wiphy_info_data *data;
	int result_finished;
};

static int wiphy_info_handler_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
	struct nrf_wifi_event_get_wiphy *wiphy_info = (struct nrf_wifi_event_get_wiphy *)msg;
	struct cfg_wiphy_info_arg *_arg = arg;
        struct wiphy_info_data *info = _arg->data;
        struct wpa_driver_nl80211_data *drv = _arg->drv;
	struct wpa_driver_capa *capa = info->capa;

	drv->wiphy_idx = wiphy_info->umac_hdr.ids.nrf_wifi_wiphy_idx;

	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_WIPHY_NAME)
		os_strlcpy(drv->phyname, wiphy_info->wiphy_name, NRF_WIFI_INDEX_IDS_WIPHY_NAME);
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_MAX_NUM_SCAN_SSIDS)
		capa->max_scan_ssids = wiphy_info->max_scan_ssids;
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_NUM_SCHED_SCAN_SSIDS)
		capa->max_sched_scan_ssids = wiphy_info->max_sched_scan_ssids;
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_MAX_MATCH_SETS)
		capa->max_match_sets = wiphy_info->max_match_sets;
//	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_MAC_ACL_MAX)
//		capa->max_acl_mac_addrs = wiphy_info->not_available
	
	wiphy_info_supported_iftypes_cfg(info, wiphy_info);
	wiphy_info_iface_comb_cfg(info, wiphy_info);
	wiphy_info_supp_cmds_cfg(info, wiphy_info);
	wiphy_info_cipher_suites_cfg(info, wiphy_info);

	if (wiphy_info->get_wiphy_flags  & NRF_WIFI_EVENT_GET_WIPHY_OFFCHANNEL_TX_OK) {
		capa->flags |= WPA_DRIVER_FLAGS_OFFCHANNEL_TX;
		wpa_printf(MSG_DEBUG, "nl80211: Using driver-based "
			   "off-channel TX");
	}
	if (wiphy_info->get_wiphy_flags  & NRF_WIFI_EVENT_GET_WIPHY_SUPPORTS_FW_ROAM) {
		wpa_printf(MSG_DEBUG, "nl80211: Using driver-based roaming");
		capa->flags |= WPA_DRIVER_FLAGS_BSS_SELECTION;
	}
	
	wiphy_info_max_roc_cfg(capa, wiphy_info);

	if (wiphy_info->get_wiphy_flags & NRF_WIFI_EVENT_GET_WIPHY_AP_UAPSD)
		capa->flags |= WPA_DRIVER_FLAGS_AP_UAPSD;
	
	wiphy_info_tdls_cfg(capa, wiphy_info);

	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_HAVE_AP_SME)
		info->device_ap_sme = 1;
	
	wiphy_info_feature_flags_cfg(info, capa, wiphy_info);
	
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_EXTENDED_FEATURES)
		wiphy_info_ext_feature_flags_cfg(info, capa, wiphy_info);

	wiphy_info_probe_resp_offload_cfg(capa, wiphy_info);


	if ((wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_EXTENDED_CAPABILITIES) &&
	    drv->extended_capa == NULL) {
		drv->extended_capa =
			os_malloc(wiphy_info->extended_capabilities_len);
		if (drv->extended_capa) {
			os_memcpy(drv->extended_capa,
				  wiphy_info->extended_capabilities,
	...
	wiphy_info_extended_capab_cfg(drv, wiphy_info);
	
	if (wiphy_info->params_valid & NRF_WIFI_GET_WIPHY_VALID_MAX_AP_ASSOC_STA)
		capa->max_stations = wiphy_info->max_ap_assoc_sta;
...
{
#ifdef CFG_INTERFACE
	struct nrf_wifi_cmd_get_wiphy *get_wiphy_cmd;
	struct cfg_wiphy_info_arg arg;
	int ret;
#else      
	u32 feat;
	struct nl_msg *msg;
...
#ifdef CFG_INTERFACE
	get_wiphy_cmd = os_zalloc(sizeof (*get_wiphy_cmd));
	...
	if(!get_wiphy_cmd) {
        ...
        get_wiphy_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_WIPHY;
        get_wiphy_cmd->umac_hdr.ids.wdev_id = 0; //Hard Coded
        get_wiphy_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        arg.result_finished = 1;
	arg.drv = drv;
        arg.data = info;
	ret = send_and_recv_msgs_cfg(drv, wiphy_info_handler_cfg, &arg, get_wiphy_cmd,
			sizeof(*get_wiphy_cmd), NRF_WIFI_UMAC_EVENT_NEW_WIPHY);
        if (get_wiphy_cmd)
                os_free(get_wiphy_cmd);
...
static void phy_info_ht_capa_cfg(struct hostapd_hw_modes *mode, u16 capa,
			     u8 ampdu_factor,
			     u8 ampdu_density,
			     struct nrf_wifi_event_mcs_info *mcs_set)
{
	if (capa)
		mode->ht_capab = (capa);

	if (ampdu_factor)
		mode->a_mpdu_params |= (ampdu_factor) & 0x03;

	if (ampdu_density)
		mode->a_mpdu_params |= (ampdu_density) << 2;

	if (mcs_set ) {
		os_memcpy(mode->mcs_set, mcs_set, 16);
	}
...
static void phy_info_vht_capa_cfg(struct hostapd_hw_modes *mode,
				  unsigned int capa,
				  struct nrf_wifi_event_vht_mcs_info *vht_mcs_set)
{
	if (capa)
		mode->vht_capab = (capa);
...
static void phy_info_freq_cfg(struct hostapd_hw_modes *mode,
			  ...
			  struct nrf_wifi_event_channel *chnl_info)
{
	u8 channel;
	...
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_DISABLED) 
		chan->flag |= HOSTAPD_CHAN_DISABLED;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_IR) 
		chan->flag |= HOSTAPD_CHAN_NO_IR;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_RADAR) 
		chan->flag |= HOSTAPD_CHAN_RADAR;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_INDOOR_ONLY) 
		chan->flag |= HOSTAPD_CHAN_INDOOR_ONLY;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_GO_CONCURRENT) 
		chan->flag |= HOSTAPD_CHAN_GO_CONCURRENT;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_10MHZ) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_10;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_20MHZ) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_20;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_HT40_PLUS) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_40P;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_HT40_MINUS) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_40M;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_80MHZ) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_80;
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_FLAG_FREQUENCY_ATTR_NO_160MHZ) 
		chan->allowed_bw &= ~HOSTAPD_CHAN_WIDTH_160;

	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_DFS_VALID) {
		enum nl80211_dfs_state state = (chnl_info->dfs_state);
	...
	if (chnl_info->nrf_wifi_flags & NRF_WIFI_CHAN_DFS_CAC_TIME_VALID) {
		chan->dfs_cac_ms = (chnl_info->nrf_wifi_time);
	}
...
static int phy_info_freqs_cfg(struct phy_info_arg *phy_info,
			  ...
			  struct nrf_wifi_event_supported_band *band_info)
{
	int new_channels = 0;
	struct hostapd_channel_data *channel;
	int idx;

	new_channels = band_info->nrf_wifi_n_channels;
	if (!new_channels)
		return NL_OK;
		...
		struct nrf_wifi_event_supported_band *band_info)
{
	...
	mode->num_rates = band_info->nrf_wifi_n_bitrates;
	if (!mode->num_rates)
		return NL_OK;
	...
	for (int i = 0; i < mode->num_rates; i++) {
		if (!band_info->bitrates[i].nrf_wifi_bitrate)
			continue;
		mode->rates[idx] = band_info->bitrates[i].nrf_wifi_bitrate;
		idx++;
	}
		...
		struct nrf_wifi_event_supported_band *band_info)
{
	...
	phy_info_ht_capa_cfg(mode, band_info->ht_cap.nrf_wifi_cap,
			 band_info->ht_cap.nrf_wifi_ampdu_factor,
			 band_info->ht_cap.nrf_wifi_ampdu_density,
			 &band_info->ht_cap.mcs);
	phy_info_vht_capa_cfg(mode, band_info->vht_cap.nrf_wifi_cap,
...
struct cfg_hw_feature_info_arg {
        struct phy_info_arg *result;
	int result_finished;
};

static int phy_info_handler_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
	struct nrf_wifi_event_get_wiphy *wiphy_info = (struct nrf_wifi_event_get_wiphy *)msg;
        struct cfg_hw_feature_info_arg *_arg = arg;
        struct phy_info_arg *info = _arg->result;

	for (int i = 0; i < NRF_WIFI_EVENT_GET_WIPHY_NUM_BANDS; i++) {
		 struct nrf_wifi_event_supported_band *band_info = &wiphy_info->sband[i];
		 if (band_info->channels) {
			int res = phy_info_band_cfg(info, &wiphy_info->sband[i]);
			if (res != NL_OK) {
				return res;
			}
		}
...
#ifdef CFG_INTERFACE
static void nl80211_reg_rule_sec_cfg(struct nrf_wifi_reg_rules *reg_rules, 
				 struct phy_info_arg *results)
{
	u32 start, end, max_bw;
	u16 m;

	if (!(reg_rules->valid_fields & FREQ_RANGE_START_VALID) ||
	    !(reg_rules->valid_fields & FREQ_RANGE_END_VALID) ||
	    !(reg_rules->valid_fields & FREQ_RANGE_MAX_BW_VALID))
		return;

	start = reg_rules->freq_range_start / 1000;
	end = reg_rules->freq_range_end / 1000;
	max_bw = reg_rules->freq_range_max_bw / 1000;
...
#ifdef CFG_INTERFACE
static void nl80211_reg_rule_vht_cfg(struct nrf_wifi_reg_rules *reg_rules,
                                 struct phy_info_arg *results)
{
        u32 start, end, max_bw;
        u16 m;
...
#ifdef CFG_INTERFACE
static void nl80211_reg_rule_6ghz_cfg(struct nrf_wifi_reg_rules *reg_rules,
                                 struct phy_info_arg *results)
{
        u32 start, end, max_bw;
        u16 m;
...
struct cfg_reg_info_arg {
        struct phy_info_arg *result;
	int result_finished;
};

static int nl80211_get_reg_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
	struct  nrf_wifi_reg *reg_info = (struct nrf_wifi_reg *)msg;
	struct cfg_reg_info_arg *_arg = arg;
	struct phy_info_arg *results = _arg->result;
	char alpha2[3];

	if (reg_info->valid_fields & NRF_WIFI_CMD_SET_REG_DFS_REGION_VALID) {
		enum nl80211_dfs_regions dfs_domain;
		dfs_domain = reg_info->dfs_region;

		nl80211_set_dfs_domain(dfs_domain, &results->dfs_domain);
		wpa_printf(MSG_DEBUG, "nl80211: -------1-----Regulatory information - country=%s (%s)",
			   (char *) reg_info->nrf_wifi_alpha2,
			   dfs_domain_name(dfs_domain));
	} else {
		os_memcpy(alpha2, reg_info->nrf_wifi_alpha2, 2);
		alpha2[2] = '\0';
		wpa_printf(MSG_DEBUG, "nl80211: --------2-----Regulatory information - country=%s country=%s",
			   (char *) reg_info->nrf_wifi_alpha2, alpha2);
	}

	if (reg_info->valid_fields & NRF_WIFI_CMD_SET_REG_RULES_VALID) {
		for (int i = 0; i < reg_info->num_reg_rules; i++) {
			u32 start, end, max_eirp = 0, max_bw = 0, flags = 0;

			if (!(reg_info->nrf_wifi_reg_rules[i].valid_fields & FREQ_RANGE_START_VALID) ||
			    !(reg_info->nrf_wifi_reg_rules[i].valid_fields & FREQ_RANGE_END_VALID))
				continue;
	
			start = (reg_info->nrf_wifi_reg_rules[i].freq_range_start) / 1000;
			end = (reg_info->nrf_wifi_reg_rules[i].freq_range_end) / 1000;
			if (reg_info->nrf_wifi_reg_rules[i].valid_fields & POWER_RULE_MAX_EIRP_VALID) {
				if (reg_info->nrf_wifi_reg_rules[i].pwr_max_eirp)
					max_eirp = (reg_info->nrf_wifi_reg_rules[i].pwr_max_eirp) / 100;
			}

			if (reg_info->nrf_wifi_reg_rules[i].valid_fields & FREQ_RANGE_MAX_BW_VALID) {
				if (reg_info->nrf_wifi_reg_rules[i].freq_range_max_bw)
					max_bw = (reg_info->nrf_wifi_reg_rules[i].freq_range_max_bw) / 1000;
			}

			if (reg_info->nrf_wifi_reg_rules[i].valid_fields & REG_RULE_FLAGS_VALID) {
				if (reg_info->nrf_wifi_reg_rules[i].rule_flags)
					flags = reg_info->nrf_wifi_reg_rules[i].rule_flags;
			}
	...
	for (int i = 0; i < reg_info->num_reg_rules; i++)
	{
		nl80211_reg_rule_sec_cfg(&reg_info->nrf_wifi_reg_rules[i], results);
	}
	for (int i = 0; i < reg_info->num_reg_rules; i++)
	{
		nl80211_reg_rule_vht_cfg(&reg_info->nrf_wifi_reg_rules[i], results);
	}
	
	for (int i = 0; i < reg_info->num_reg_rules; i++)
	{
		nl80211_reg_rule_6ghz_cfg(&reg_info->nrf_wifi_reg_rules[i], results);
	}
					...
					struct phy_info_arg *results)
{
	...
	wpa_printf(MSG_DEBUG, "CALLING -------------------------NRF_WIFI_UMAC_CMD_GET_REG-----------------------\n");
        reg_get_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_REG;
        reg_get_cmd->umac_hdr.ids.wdev_id = 0; //Hard Coded
        reg_get_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        arg.result_finished = 1;
        arg.result = results;

        ret = send_and_recv_msgs_cfg(drv, nl80211_get_reg_cfg, &arg, reg_get_cmd,
			    ...
			    u8 *dfs_domain)
{
...
#ifdef CFG_INTERFACE
        struct nrf_wifi_cmd_get_wiphy *get_wiphy_cmd;
        struct cfg_hw_feature_info_arg arg;
        int ret = -1;
#else	
	int nl_flags = 0;
	struct nl_msg *msg;
	...
	*dfs_domain = 0;
        ...
        arg.result_finished = 1;
        arg.result = &result;
	ret = send_and_recv_msgs_cfg(drv, phy_info_handler_cfg, &arg, get_wiphy_cmd,
			sizeof(*get_wiphy_cmd), NRF_WIFI_UMAC_EVENT_NEW_WIPHY);
out:
        if (get_wiphy_cmd)
                os_free(get_wiphy_cmd);
