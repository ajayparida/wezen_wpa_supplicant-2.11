...
struct no_cfg_cb_info {
        struct dl_list list;
	int (*cfg_cb)(void *arg, void *msg, int len, int next_result);
	void *cfg_cb_data;
	int event_id;
};
...
struct nl80211_global {
	...
	int cfg_ifindex;
	struct	dl_list cfg_cbs_list;
	u64 remain_on_channel_cookie;
	u64 mgmt_tx_cmd_frame_cookie;
	struct dl_list cookie_list;
	struct bss_info **res;
	int res_num;
#endif
	struct nl_cb *nl_cb;
...
#ifdef CFG_INTERFACE
#ifdef BSS_OPTIMIZATION
	struct display_data_info *display_data;
#endif
#endif
};
...
int send_and_recv_msgs_cfg(struct wpa_driver_nl80211_data *drv,
                       int (*valid_handler)(void *, void *, int len, int next_data),
                       void *cb_data,
                       void *valid_data,
                       int data_len,
		       int event_id);
