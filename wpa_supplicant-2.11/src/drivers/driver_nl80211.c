...
#ifdef CFG_INTERFACE
static int send_and_recv_cfg(struct nl80211_global *global,
             void *data,
             int data_len,
             int event_wait)
{
    return cfg_netlink_send(global, data, data_len, event_wait);
}

int send_and_recv_msgs_cfg(struct wpa_driver_nl80211_data *drv,
               int (*valid_handler)(void *, void *, int len, int next_result),
               void *cb_data,
               void *valid_data,
               int data_len,
               int event_id)
{
#define MSG_DO_NOT_WAIT 0
#define MSG_WAIT 1
    if (valid_handler)  {
        struct no_cfg_cb_info *cb_info = NULL;
            cb_info = os_zalloc(sizeof(*cb_info));
            if (!cb_info)
                    return -ENOMEM;

            cb_info->cfg_cb = valid_handler;
        cb_info->event_id = event_id;
        cb_info->cfg_cb_data = cb_data;
            dl_list_add(&drv->global->cfg_cbs_list,  &cb_info->list);
        return send_and_recv_cfg(drv->global, valid_data, data_len, MSG_WAIT);
    }

    return send_and_recv_cfg(drv->global, valid_data, data_len, MSG_DO_NOT_WAIT);
}
#endif
...
struct cfg_netdev_info_arg {
        struct wiphy_idx_data *data;
    int result_finished;
};

static int netdev_info_handler_cfg(void *arg, void *msg,
        int len, int next_avail_data_not_used)
{
    struct nrf_wifi_interface_info *intf_info =
        (struct nrf_wifi_interface_info *)msg;
    struct cfg_netdev_info_arg *_arg = arg;
    struct wiphy_idx_data_cfg *info = _arg->data;

    info->wiphy_idx = intf_info->umac_hdr.ids.nrf_wifi_wiphy_idx;
    info->nlmode = intf_info->nrf_wifi_iftype;
    os_memcpy(info->macaddr, intf_info->nrf_wifi_eth_addr, ETH_ALEN);
...
{
#ifdef CFG_INTERFACE
    struct nrf_wifi_cmd_get_interface *get_intf_cmd;
    int ret = -1;
    struct cfg_netdev_info_arg arg;
    struct wiphy_idx_data_cfg data = {
        .wiphy_idx = -1,
        .macaddr = {0},
    };
#else
...
#endif
#ifdef CFG_INTERFACE
	get_intf_cmd = os_zalloc(sizeof(*get_intf_cmd));
    if(!get_intf_cmd) {
    	wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
    }

	get_intf_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_INTERFACE;
    get_intf_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
    get_intf_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    arg.data = &data;
    arg.result_finished = 1;

    ret = send_and_recv_msgs_cfg(bss->drv, netdev_info_handler_cfg, &arg, get_intf_cmd,
            sizeof(*get_intf_cmd), NRF_WIFI_UMAC_EVENT_NEW_INTERFACE);

    os_free(get_intf_cmd);
out:
    if (ret == 0)
...
static int nl80211_get_macaddr(struct i802_bss *bss)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_GET_INTERFACE command\n", __func__);
    ret = send_and_recv_msgs_cfg(bss->drv, netdev_info_handler_cfg, &arg, get_intf_cmd,
            sizeof(*get_intf_cmd), NRF_WIFI_UMAC_EVENT_NEW_INTERFACE);
...
#ifdef CFG_INTERFACE
struct cfg_chan_info_arg {
        struct wpa_channel_info *data;
    int result_finished;
};

static int get_channel_info_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
    struct nrf_wifi_interface_info *intf_info = (struct nrf_wifi_interface_info *)msg;
    struct cfg_chan_info_arg *_arg = arg;

    struct wpa_channel_info *chan_info = _arg->data;
    struct nrf_wifi_chan_definition *chan_def_info =( struct nrf_wifi_chan_definition *)(&intf_info->chan_def);

    os_memset(chan_info, 0, sizeof(struct wpa_channel_info));
    chan_info->chanwidth = CHAN_WIDTH_UNKNOWN;

    chan_info->frequency = chan_def_info->chan.center_frequency;
    chan_info->chanwidth = convert2width(chan_def_info->width);

    int channel_flags = chan_def_info->chan.nrf_wifi_flags;
    ...
    chan_info->center_frq1 = chan_def_info->center_frequency1;
    chan_info->center_frq2 = chan_def_info->center_frequency2;
...
static int nl80211_channel_info(void *priv, struct wpa_channel_info *ci)
{
    ...
    ret = send_and_recv_msgs_cfg(bss->drv, get_channel_info_cfg, &arg, get_intf_cmd,
            sizeof(*get_intf_cmd), NRF_WIFI_UMAC_EVENT_NEW_INTERFACE);
...
static int wpa_driver_nl80211_set_country(void *priv, const char *alpha2_arg)
{
	...
	char alpha2[3];
#ifdef CFG_INTERFACE
        struct nrf_wifi_cmd_req_set_reg *reg_set_cmd;
        int ret;
#else
	struct nl_msg *msg;
	...
	alpha2[2] = '\0';
#ifdef CFG_INTERFACE
    reg_set_cmd = os_zalloc(sizeof (*reg_set_cmd));
    if (!reg_set_cmd) {
		wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
    }
    ...
    reg_set_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_REQ_SET_REG;
    reg_set_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard Coded
    reg_set_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    reg_set_cmd->valid_fields = NRF_WIFI_CMD_REQ_SET_REG_ALPHA2_VALID;
    os_strlcpy(reg_set_cmd->nrf_wifi_alpha2, alpha2, 3);

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_REQ_SET_REG command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, reg_set_cmd,
            sizeof(*reg_set_cmd), -1);
...
#ifdef CFG_INTERFACE
struct cfg_reg_country_info_arg {
    char *data;
    int result_finished;
};

static int nl80211_get_country_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
    struct nrf_wifi_reg *reg_info = (struct nrf_wifi_reg *)msg;
        struct cfg_reg_country_info_arg *_arg = arg;
        unsigned char *alpha2 = _arg->data;

    if (!(reg_info->valid_fields & NRF_WIFI_CMD_SET_REG_ALPHA2_VALID)) {
        wpa_printf(MSG_DEBUG, "nl80211: No country information available");
        return NL_SKIP;
    }
    os_strlcpy(alpha2, reg_info->nrf_wifi_alpha2, 3);
...
static int wpa_driver_nl80211_get_country(void *priv, char *alpha2)
{
	...
	struct wpa_driver_nl80211_data *drv = bss->drv;
#ifdef CFG_INTERFACE
    struct nrf_wifi_reg *reg_get_cmd;
    struct cfg_reg_country_info_arg  arg;
#else
	struct nl_msg *msg;
#endif
	int ret;
#ifdef CFG_INTERFACE
        reg_get_cmd = os_zalloc(sizeof (*reg_get_cmd));
        if(!reg_get_cmd) {
                wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
        }

        reg_get_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_REG;
        reg_get_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard Coded
        reg_get_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        arg.result_finished = 1;
        arg.data = alpha2;

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_GET_REG command\n", __func__);

    ret = send_and_recv_msgs_cfg(drv, nl80211_get_country_cfg, &arg, reg_get_cmd,
            sizeof(*reg_get_cmd), NRF_WIFI_UMAC_EVENT_GET_REG);
...
#ifdef CFG_INTERFACE
int nrf_wifi_cfg80211_mgmt_frame_reg(struct wpa_driver_nl80211_data *drv, int ifindex,
                      struct nrf_wifi_umac_mgmt_frame_info *frame_info)
{
        struct nrf_wifi_umac_cmd_mgmt_frame_reg *frame_reg_cmd = NULL;
        int ret = -1;
        frame_reg_cmd = os_zalloc(sizeof(*frame_reg_cmd));
        if (!frame_reg_cmd) {
                wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
        }
        ...
        frame_reg_cmd->umac_hdr.ids.wdev_id = 0; //TODO:Hard Coded
        frame_reg_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        os_memcpy(&frame_reg_cmd->info,
                  frame_info,
                  sizeof(frame_reg_cmd->info));

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_REGISTER_FRAME command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, frame_reg_cmd,
            sizeof(*frame_reg_cmd), -1);


    os_free(frame_reg_cmd);
        return ret;
}
#endif
				  ...
				  bool multicast)
{
	...
	wpa_printf(MSG_DEBUG,
        ...
        frame_info = os_zalloc(sizeof(*frame_info));
        ...
        os_memcpy(frame_info->frame_match.frame_match,
                  match, match_len);

    ret = nrf_wifi_cfg80211_mgmt_frame_reg(drv, bss->ifindex, frame_info);
...
#ifdef NOTYET
    if (start) {
        start_p2p_dev_cmd = os_zalloc(sizeof(*start_p2p_dev_cmd));
            ...
            start_p2p_dev_cmd->umac_hdr.ids.wdev_id = bss->wdev_id; //CHECK IT
            start_p2p_dev_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        ret = send_and_recv_msgs_cfg(bss->drv, NULL, NULL, start_p2p_dev_cmd,
                sizeof(*start_p2p_dev_cmd), -1);
    } else {
            stop_p2p_dev_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_STOP_P2P_DEVICE;
            stop_p2p_dev_cmd->umac_hdr.ids.wdev_id = bss->wdev_id; //CHECK IT
            stop_p2p_dev_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        ret = send_and_recv_msgs_cfg(bss->drv, NULL, NULL, stop_p2p_dev_cmd,
                sizeof(*stop_p2p_dev_cmd), -1);

    }
    if (ret)
                wpa_printf(MSG_DEBUG, "%s: p2p command  failed\n", __func__);
out:
    if (start_p2p_dev_cmd)
        os_free(start_p2p_dev_cmd);

    if (stop_p2p_dev_cmd)
        os_free(stop_p2p_dev_cmd);
#endif
#else
					 ...
					 int link_id)
{
	...
	nl80211_put_wiphy_data_ap(bss);
	#ifdef CFG_INTERFACE
        stop_ap_cmd = os_zalloc(sizeof(*stop_ap_cmd));
        if(!stop_ap_cmd) {
                wpa_printf(MSG_ERROR,"nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
        return -ENOBUFS;
        }
        stop_ap_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_STOP_AP;
        stop_ap_cmd->umac_hdr.ids.wdev_id = 0; //TODO: hardcoded to 0
        stop_ap_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_STOP_AP command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, stop_ap_cmd,
            sizeof(*stop_ap_cmd), -1);
        if (ret < 0) {
                wpa_printf(MSG_ERROR, "nl80211: %s: send_and_recv_msgs_cfg  failed\n", __func__);
                goto out;
        }
//        bss->beacon_set = 0;
out:
...
#ifdef CFG_INTERFACE
int nrf_wifi_fmac_add_key(struct wpa_driver_nl80211_data *drv,
              int ifindex,
              ...
              const u8 *addr)
{
    ...
    key_cmd = os_zalloc(sizeof(*key_cmd));
    if(!key_cmd) {
                wpa_printf(MSG_ERROR, "%s: Unable to allocate memory\n", __func__);
            return -ENOBUFS;
    }

	key_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_NEW_KEY;
	key_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard Coded
	key_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    os_memcpy(&key_cmd->key_info,
              key_info,
              sizeof(key_cmd->key_info));

    if (addr) {
		os_memcpy(key_cmd->mac_addr,
				  addr,
                  ETH_ALEN);
                  key_cmd->valid_fields |= NRF_WIFI_CMD_KEY_MAC_ADDR_VALID;
	}
	wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_NEW_KEY command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, key_cmd,
            sizeof(*key_cmd), -1);
    if (ret < 0)
                wpa_printf(MSG_ERROR, "%s: send_and_recv_msgs_cfg failed\n", __func__);

    os_free(key_cmd);
    return ret;
}

int nrf_wifi_fmac_del_key(struct wpa_driver_nl80211_data *drv,
              int ifindex,
              struct nrf_wifi_umac_key_info *key_info,
              const u8 *mac_addr)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_DEL_KEY command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, key_cmd,
            sizeof(*key_cmd), -1);
	if (ret < 0)
		wpa_printf(MSG_ERROR, "%s: send_and_recv_msgs_cfg failed\n", __func__);
...
int nrf_wifi_fmac_set_key(struct wpa_driver_nl80211_data *drv,
              int ifindex,
              struct nrf_wifi_umac_key_info *key_info)
{
    ...
    set_key_cmd = os_zalloc(sizeof(*set_key_cmd));
    if(!set_key_cmd) {
		wpa_printf(MSG_ERROR, "%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
	}
    ...
    set_key_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard Coded
    set_key_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    os_memcpy(&set_key_cmd->key_info,
              key_info,
              sizeof(set_key_cmd->key_info));

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_KEY command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, set_key_cmd,
            sizeof(*set_key_cmd), -1);
    if (ret < 0)
        wpa_printf(MSG_ERROR, "%s: send_and_recv_msgs_cfg failed\n", __func__);
...
static int wpa_driver_nl80211_set_key_cfg(const char *ifname, struct i802_bss *bss,
                      ...
                      const u8 *key, size_t key_len)
{
    ...
    if (key_idx == -1) {
        ...
        key_info = os_zalloc(sizeof(*key_info));
        if(!key_info) {
        wpa_printf(MSG_ERROR,"%s: Unable to allocate memory\n", __func__);
            return -ENOBUFS;
        }
#if 0
#TODO: set_pmk is not supported in UMAC
    if (alg == WPA_ALG_PMK &&
    ...
    if (alg != WPA_ALG_NONE) {
		...
		os_memcpy(key_info->key.nrf_wifi_key,
                  key,
                  key_len);
        key_info->key.nrf_wifi_key_len = key_len;
        key_info->cipher_suite = suite;

        key_info->valid_fields |= NRF_WIFI_CIPHER_SUITE_VALID |
                      NRF_WIFI_KEY_VALID;
    ...
    if (seq && seq_len) {
        os_memcpy(key_info->seq.nrf_wifi_seq,
                  seq,
                  seq_len);
...
#if 0
    pairwise = 1;
        key_info->key_type = pairwise ? NRF_WIFI_KEYTYPE_PAIRWISE : NRF_WIFI_KEYTYPE_GROUP;
    key_info->valid_fields |= NRF_WIFI_KEY_TYPE_VALID;
#endif

    if (alg == WPA_ALG_NONE) {
        wpa_printf(MSG_DEBUG, "nl80211: %s: nrf_wifi_fmac_del_key, valid_fields = %d\n", __func__, key_info->valid_fields);
        ret = nrf_wifi_fmac_del_key(drv ,ifindex, key_info, mac_addr);
        if (ret < 0)
            wpa_printf(MSG_ERROR, "nl80211: %s: nrf_wifi_fmac_del_key failed\n", __func__);
    } else {
        wpa_printf(MSG_DEBUG, "nl80211: %s: nrf_wifi_fmac_add_key, valid_fields = %d \n", __func__, key_info->valid_fields);  
        ret = nrf_wifi_fmac_add_key(drv, ifindex, key_info, mac_addr);
        if (ret < 0)
            wpa_printf(MSG_ERROR, "nl80211: %s: nrf_wifi_fmac_add_key failed\n", __func__);
    }
    ...
    os_memset(key_info, 0, sizeof(*key_info));
    key_info->key_idx = key_idx;
    key_info->valid_fields |= NRF_WIFI_KEY_IDX_VALID;
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s: calling nrf_wifi_fmac_set_key, valid_fields = %d\n", __func__,key_info->valid_fields);
    ret = nrf_wifi_fmac_set_key(drv, ifindex, key_info);
    if (ret < 0)
        wpa_printf(MSG_ERROR, "nl80211: %s: nrf_wifi_fmac_set_key failed\n", __func__);
out:
    os_free(key_info);
    return ret;
}
...
#ifdef CFG_INTERFACE
static int cfg_add_key(struct nrf_wifi_umac_key_info *key_info, enum wpa_alg alg,
                      ...
                      const u8 *key, size_t key_len)
{
    ...
    if (key && key_len) {
        os_memcpy(key_info->key.nrf_wifi_key, key, key_len);
        key_info->key.nrf_wifi_key_len = key_len;
    }
    if (seq && seq_len) {
        os_memcpy(key_info->seq.nrf_wifi_seq, seq, seq_len);
        key_info->seq.nrf_wifi_seq_len = seq_len;
    }
    return 0;
}
...
#ifdef CFG_INTERFACE
int nrf_wifi_cfg80211_deauth(struct wpa_driver_nl80211_data *drv,
        int ifindex, const u8 *addr, u16 reason_code,
                             int local_state_change)
{
        ...
        deauth_info = os_zalloc(sizeof(*deauth_info));
        if (!deauth_info) {
                wpa_printf(MSG_DEBUG,"%s: Unable to allocate memory\n", __func__);
            return -ENOBUFS;
        }
        ...
        os_memcpy(deauth_info->mac_addr,
               addr,
               ETH_ALEN);

        if (local_state_change)
                deauth_info->nrf_wifi_flags |= NRF_WIFI_CMD_MLME_LOCAL_STATE_CHANGE;

	    deauth_cmd = os_zalloc(sizeof(*deauth_cmd));
        if (!deauth_cmd) {
                wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
                goto out;
		}
        ...
        deauth_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard coded to 0
        deauth_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        os_memcpy(&deauth_cmd->info,
                          deauth_info,
       ...
       //                                sizeof(deauth_info->mac_addr)))
    	...
    	wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_DEAUTHENTICATE command\n", __func__);
	    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, deauth_cmd,	
    	        sizeof(*deauth_cmd), -1);
out:
        if (deauth_cmd)
                os_free(deauth_cmd);
					     ...
					     const u8 *addr, u16 reason_code)
{
	...
	nl80211_mark_disconnected(drv);
#ifdef CFG_INTERFACE
    ret = nrf_wifi_cfg80211_deauth(drv, bss->ifindex, addr, reason_code, 0);
#else
	...
	if (ret == 0)
#ifdef CFG_INTERFACE
		drv->ignore_next_local_deauth = drv->associated && (ret == 0);
#else
...
#ifdef CFG_INTERFACE
int nrf_wifi_cfg80211_auth(struct wpa_driver_nl80211_data *drv,
        struct nrf_wifi_umac_auth_info *auth_info)
{
    struct nrf_wifi_umac_cmd_auth *auth_cmd = NULL;
    int ret = -1;

    auth_cmd = os_zalloc(sizeof(*auth_cmd));
    ...
    auth_cmd->umac_hdr.ids.wdev_id = 0; //TODO: HArd coded
    ...
    os_memcpy(&auth_cmd->info,
                  auth_info,
                  sizeof(auth_cmd->info));


    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_AUTHENTICATE command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, auth_cmd,
            sizeof(*auth_cmd), -1);

    if (ret < 0) {
        wpa_printf(MSG_DEBUG, "%s: nrf_wifi_fmac_auth failed\n", __func__);
    }
...
retry:
...
#ifdef CFG_INTERFACE
    global = drv->global;
    global->cfg_ifindex = drv->ifindex;
    auth_info = os_zalloc(sizeof(*auth_info));
	...
	for (i = 0; i < 4; i++) {
		if (!params->wep_key[i])
			continue;
#ifdef CFG_INTERFACE
        wpa_driver_nl80211_set_key_cfg(bss->ifname, bss, WPA_ALG_WEP,
                                           NULL, i,
        ...
        if (params->wep_tx_keyidx != i)
            continue;
        if (cfg_add_key(&auth_info->key_info, WPA_ALG_WEP, i, 1, NULL, 0,
                               params->wep_key[i], params->wep_key_len[i]) < 0)
            goto fail;
        if (auth_info->key_info.key.nrf_wifi_key_len > 0) {
            auth_info->key_info.valid_fields |= NRF_WIFI_KEY_IDX_VALID;
            auth_info->key_info.valid_fields |= NRF_WIFI_KEY_VALID;
            auth_info->key_info.valid_fields |= NRF_WIFI_CIPHER_SUITE_VALID;
        }
#else
		p.key_idx = i;
...
#ifdef CFG_INTERFACE
        os_memcpy(auth_info->nrf_wifi_bssid,
                          params->bssid,
                          ETH_ALEN);
#else
	if (params->bssid) {
...
#ifdef CFG_INTERFACE
         os_memcpy(auth_info->ssid.nrf_wifi_ssid,
                          params->ssid,
                          params->ssid_len);
...
#ifdef CFG_INTERFACE
    if (params->ie) {
        os_memcpy(auth_info->bss_ie.ie,
                      params->ie,
                      params->ie_len);
    } else {
#if 0
        struct wpa_bss *curr_bss = wpa_bss_get(drv->ctx, params->bssid, params->ssid, params->ssid_len);
	...
	if (params->auth_data) {
        ...
        os_memcpy(auth_info->sae.sae_data,
                        params->auth_data,
                        params->auth_data_len);
#else
...
#endif
#ifdef CFG_INTERFACE
    ret = nrf_wifi_cfg80211_auth(drv, auth_info);
#else
	ret = send_and_recv_cmd(drv, msg);
	msg = NULL;
#endif
	if (ret) {
		...
		if (ret == -ENOENT && params->freq && !is_retry) {
			...
			wpa_printf(MSG_DEBUG, "nl80211: Trigger single "
				   ...
				   "entry");
#ifdef CFG_INTERFACE
            ret = wpa_driver_nl80211_scan_cfg(bss, &scan);
#else
...
#ifdef CFG_INTERFACE
    if (beacon_set) {
        set_bcn_cmd = os_zalloc(sizeof(*set_bcn_cmd));
        if (!set_bcn_cmd) {
            wpa_printf(MSG_ERROR,"nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
            goto out;
        }
        ...
        set_bcn_cmd->umac_hdr.ids.wdev_id = 0; //TODO:HArd coded
        set_bcn_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        if (params->head_len > 0) {
            os_memcpy(set_bcn_cmd->info.beacon_data.head,
                  params->head,
                  params->head_len);
            set_bcn_cmd->info.beacon_data.head_len = params->head_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Beacon head",
                    set_bcn_cmd->info.beacon_data.head,
                    set_bcn_cmd->info.beacon_data.head_len);
        } else {
            wpa_printf(MSG_ERROR, "nl80211: %s: Beacon Head len is zero. %d\n", __func__, __LINE__);
            goto out;
        }

        if (params->tail_len > 0) {
            os_memcpy(set_bcn_cmd->info.beacon_data.tail,
                  params->tail,
                  params->tail_len);
            set_bcn_cmd->info.beacon_data.tail_len = params->tail_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Beacon tail",
                    set_bcn_cmd->info.beacon_data.tail,
                    set_bcn_cmd->info.beacon_data.tail_len);
        }
        if (params->proberesp_len > 0) {
            os_memcpy(set_bcn_cmd->info.beacon_data.probe_resp,
                  params->proberesp,
                  params->proberesp_len);
            set_bcn_cmd->info.beacon_data.probe_resp_len = params->proberesp_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Probe Resp",
                    set_bcn_cmd->info.beacon_data.probe_resp,
                    set_bcn_cmd->info.beacon_data.probe_resp_len);
        }
		// TODO: UMAC should provide these variable in their strcuture, then we can parse
        // 1. beacon_ies
        // 2. proberesp_ies
        // 3. assocresp_ies
        // 4. ftm_responder, lci , civic
        //

        wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_BEACON command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL,
                         set_bcn_cmd,
                         sizeof(*set_bcn_cmd),
                         -1);
        if (ret < 0) {
            wpa_printf(MSG_DEBUG, "nl80211: %s: send_and_recv_msgs_cfg failed\n", __func__);
            goto out;
        }

    } else {
        start_ap_cmd = os_zalloc(sizeof(*start_ap_cmd));
        if (!start_ap_cmd) {
            wpa_printf(MSG_ERROR,"nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
            goto out;
        }
        ...
        start_ap_cmd->umac_hdr.ids.wdev_id = 0;  //TODO: HardCoded
        start_ap_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;
		
        if (params->head_len > 0) {
            os_memcpy(start_ap_cmd->info.beacon_data.head,
                  params->head,
                  params->head_len);
            start_ap_cmd->info.beacon_data.head_len = params->head_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Beacon head",
                    start_ap_cmd->info.beacon_data.head,
                    start_ap_cmd->info.beacon_data.head_len);
        } else {
            wpa_printf(MSG_ERROR, "nl80211: %s: Beacon Head len is zero. %d\n", __func__, __LINE__);
            goto out;
        }
        ...
        if (params->tail_len > 0) {
            os_memcpy(start_ap_cmd->info.beacon_data.tail,
                  params->tail,
                  params->tail_len);
            start_ap_cmd->info.beacon_data.tail_len = params->tail_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Beacon tail",
                    start_ap_cmd->info.beacon_data.tail,
                    start_ap_cmd->info.beacon_data.tail_len);
        }
		 if (params->proberesp_len > 0) {
            os_memcpy(start_ap_cmd->info.beacon_data.probe_resp,
                  params->proberesp,
                  params->proberesp_len);
            start_ap_cmd->info.beacon_data.probe_resp_len = params->proberesp_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: Probe Resp",
                    start_ap_cmd->info.beacon_data.probe_resp,
                    start_ap_cmd->info.beacon_data.probe_resp_len);
        }
        // TODO: UMAC should provide these variable in their strcuture, then we can parse
        // 1. beacon_ies
        // 2. proberesp_ies
        ...
        wpa_printf(MSG_DEBUG, "nl80211: %s: beacon_interval=%d", __func__,  params->beacon_int);
        if (params->beacon_int < 10 || params->beacon_int > 10000) {
            wpa_printf(MSG_ERROR, "nl80211: %s: Invalid beacon_interval=%d", __func__,  params->beacon_int);
            goto out;
        }
        start_ap_cmd->info.beacon_interval = params->beacon_int;
        start_ap_cmd->valid_fields |= NRF_WIFI_CMD_BEACON_INFO_BEACON_INTERVAL_VALID;

        // TODO: nl80211_put_beacon_rate
        // nl80211_put_beacon_rate(msg, drv->capa.flags, params)
		
        wpa_printf(MSG_DEBUG, "nl80211: %s: dtim_period=%d", __func__,  params->dtim_period);
        start_ap_cmd->info.dtim_period = params->dtim_period;

        //TODO: replace 32 with some Define
        if (params->ssid_len == 0 || params->ssid_len > 32) {
            wpa_printf(MSG_ERROR, "nl80211: %s: Invalid SSID Len %ld", __func__, params->ssid_len);
            goto out;
        } else {
            os_memcpy(start_ap_cmd->info.ssid.nrf_wifi_ssid,
                  params->ssid,
                  params->ssid_len);
            start_ap_cmd->info.ssid.nrf_wifi_ssid_len = params->ssid_len;
            wpa_hexdump(MSG_DEBUG, "nl80211: SSID",
                    start_ap_cmd->info.ssid.nrf_wifi_ssid,
                    start_ap_cmd->info.ssid.nrf_wifi_ssid_len);
        }

        wpa_printf(MSG_DEBUG, "nl80211: %s: hidden_ssid=%d", __func__,  params->hide_ssid);
        switch (params->hide_ssid) {
            case NO_SSID_HIDING:
                wpa_printf(MSG_DEBUG, "nl80211: %s: hidden SSID not in use", __func__);
//                start_ap_cmd->info.hidden_ssid = NRF_WIFI_HIDDEN_SSID_NOT_IN_USE;
                break;
            case HIDDEN_SSID_ZERO_LEN:
                wpa_printf(MSG_DEBUG, "nl80211: %s: hidden SSID zero len", __func__);
//                start_ap_cmd->info.hidden_ssid = NRF_WIFI_HIDDEN_SSID_ZERO_LEN;
                break;
            case HIDDEN_SSID_ZERO_CONTENTS:
                wpa_printf(MSG_DEBUG, "nl80211: %s: hidden SSID zero contents", __func__);
//                start_ap_cmd->info.hidden_ssid = NRF_WIFI_HIDDEN_SSID_ZERO_CONTENTS;
                break;
			 }

        wpa_printf(MSG_DEBUG, "nl80211: %s: privacy=%d", __func__, params->privacy);
        if (params->privacy == 1)
            start_ap_cmd->info.nrf_wifi_flags = NRF_WIFI_CMD_BEACON_INFO_PRIVACY;

        wpa_printf(MSG_DEBUG, "nl80211: %s: auth_algs=0x%x", __func__, params->auth_algs);

        if ((params->auth_algs & (WPA_AUTH_ALG_OPEN | WPA_AUTH_ALG_SHARED)) ==
            (WPA_AUTH_ALG_OPEN | WPA_AUTH_ALG_SHARED)) {
            /* Leave out the attribute */
        } else if (params->auth_algs & WPA_AUTH_ALG_SHARED) {
            start_ap_cmd->info.auth_type = NRF_WIFI_AUTHTYPE_SHARED_KEY;
            start_ap_cmd->valid_fields |= NRF_WIFI_CMD_BEACON_INFO_AUTH_TYPE_VALID;
        } else {
            start_ap_cmd->info.auth_type = NRF_WIFI_AUTHTYPE_OPEN_SYSTEM;
            start_ap_cmd->valid_fields |= NRF_WIFI_CMD_BEACON_INFO_AUTH_TYPE_VALID;
        }

        //TODO: GET WPA_VERISON enum value from umac and parse it accordingly
        wpa_printf(MSG_DEBUG, "nl80211: %s: wpa_version=0x%x", __func__, params->wpa_version);
        start_ap_cmd->info.connect_common_info.wpa_versions = params->wpa_version;
        start_ap_cmd->info.connect_common_info.valid_fields |=
                NRF_WIFI_CONNECT_COMMON_INFO_WPA_VERSIONS_VALID;

        wpa_printf(MSG_DEBUG, "nl80211: %s, key_mgmt_suites=0x%x", __func__, params->key_mgmt_suites);
        if (params->key_mgmt_suites & WPA_KEY_MGMT_IEEE8021X) {
            start_ap_cmd->info.connect_common_info.akm_suites[0] = RSN_AUTH_KEY_MGMT_UNSPEC_802_1X;
            start_ap_cmd->info.connect_common_info.num_akm_suites++;
            start_ap_cmd->info.connect_common_info.valid_fields |= NRF_WIFI_CONNECT_COMMON_INFO_AKM_SUITES_VALID;
        }
		if (params->key_mgmt_suites & WPA_KEY_MGMT_PSK) {
            start_ap_cmd->info.connect_common_info.akm_suites[1] = RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X;
            start_ap_cmd->info.connect_common_info.num_akm_suites++;
            start_ap_cmd->info.connect_common_info.valid_fields |= NRF_WIFI_CONNECT_COMMON_INFO_AKM_SUITES_VALID;

        }
//TODO: HACK - multiply by sizeof(u32)
        start_ap_cmd->info.connect_common_info.num_akm_suites *= sizeof(u32);

        if (params->key_mgmt_suites & WPA_KEY_MGMT_IEEE8021X_NO_WPA &&
                (!params->pairwise_ciphers ||
             params->pairwise_ciphers & (WPA_CIPHER_WEP104 | WPA_CIPHER_WEP40))) {
            start_ap_cmd->info.connect_common_info.control_port_ether_type = ETH_P_PAE;
#if 0
            start_ap_cmd->info.connect_common_info.control_port_no_encrypt = TRUE;
#endif
            start_ap_cmd->info.connect_common_info.control_port_no_encrypt = 1;
            start_ap_cmd->info.connect_common_info.valid_fields |= NRF_WIFI_CONNECT_COMMON_INFO_CONTROL_PORT_ETHER_TYPE |
                                           NRF_WIFI_CONNECT_COMMON_INFO_CONTROL_PORT_NO_ENCRYPT;
        }
		
		 if (drv->device_ap_sme && (params->key_mgmt_suites & WPA_KEY_MGMT_SAE)) {
            //TODO: UMAC should provide cfg80211_ap_settings_flags
            wpa_printf(MSG_DEBUG, "nl80211: %s: AP supports external authentication", __func__);
        }


        wpa_printf(MSG_DEBUG, "nl80211: %s, pairwise_ciphers=0x%x", __func__, params->pairwise_ciphers);
        num_suites = wpa_cipher_to_cipher_suites(params->pairwise_ciphers,
                             suites, ARRAY_SIZE(suites));

        os_memcpy(start_ap_cmd->info.connect_common_info.cipher_suites_pairwise,
              suites,
              num_suites * sizeof(u32));
//TODO: HACK - multiply by sizeof(u32)
        start_ap_cmd->info.connect_common_info.num_cipher_suites_pairwise = num_suites * sizeof(u32);
        start_ap_cmd->info.connect_common_info.valid_fields |= NRF_WIFI_CONNECT_COMMON_INFO_CIPHER_SUITES_PAIRWISE_VALID;

        wpa_printf(MSG_DEBUG, "nl80211: %s, group_cipher=0x%x", __func__, params->group_cipher);
        suite = wpa_cipher_to_cipher_suite(params->group_cipher);
        start_ap_cmd->info.connect_common_info.cipher_suite_group = suite;
        start_ap_cmd->info.connect_common_info.valid_fields |= NRF_WIFI_CONNECT_COMMON_INFO_CIPHER_SUITE_GROUP_VALID;
#if 0
		if (params->ht_opmode != -1) {
            switch (params->smps_mode) {
                ...
                case HT_CAP_INFO_SMPS_STATIC:
                    ...
                    smps_mode = NRF_WIFI_SMPS_STATIC;
                    break;
                default:
                /* invalid - fallback to smps off */
                case HT_CAP_INFO_SMPS_DISABLED:
                    wpa_printf(MSG_DEBUG, "nl80211: SMPS mode - off");
                    smps_mode = NRF_WIFI_SMPS_OFF;
                    break;
            }
            start_ap_cmd->info.smps_mode = smps_mode;
            //TODO: Valid flag for smps_mode
        }
#endif
        //TODO: UMAC provide the strcuture
        // 1. proberesp_ies
        // 2. assocresp_ies
        //
        ...
        if (drv->capa.flags & WPA_DRIVER_FLAGS_INACTIVITY_TIMER)  {
            ...
            start_ap_cmd->info.inactivity_timeout = params->ap_max_inactivity;
            start_ap_cmd->valid_fields |= NRF_WIFI_CMD_BEACON_INFO_INACTIVITY_TIMEOUT_VALID;
        }
        ...
        if (params->p2p_go_ctwindow > 0) {
            if (drv->p2p_go_ctwindow_supported) {
                ...
                start_ap_cmd->info.p2p_go_ctwindow = params->p2p_go_ctwindow;
            start_ap_cmd->valid_fields |= NRF_WIFI_CMD_BEACON_INFO_P2P_CTWINDOW_VALID;
                //TODO:  opportunistic power save
            }

        } else {
            wpa_printf(MSG_INFO,
                   "nl80211: Driver does not support CTWindow config - ignore this parameter");
        }
#endif /* CONFIG_P2P */
        //TODO: UMAC should provide pbss variable to process the pbss data
        if (params->pbss) {
            wpa_printf(MSG_DEBUG, "nl80211: PBSS");
        }

        wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_START_AP command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL,
                         start_ap_cmd,
                         sizeof(*start_ap_cmd),
                         -1);
    ...
    link->beacon_set = 1; //TODO
    ret = -1;

    set_bss_cmd = os_zalloc(sizeof(*set_bss_cmd));
    if(!set_bss_cmd) {
        wpa_printf(MSG_ERROR,"nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
        goto out;
    }
    ...
    set_bss_cmd->umac_hdr.ids.wdev_id =  0; //TODO:HArd coded
    set_bss_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    set_bss_cmd->bss_info.ht_opmode = params->ht_opmode;
    set_bss_cmd->bss_info.nrf_wifi_cts = params->cts_protect;
    set_bss_cmd->bss_info.preamble = params->preamble;
    set_bss_cmd->bss_info.nrf_wifi_slot = params->short_slot_time;
    set_bss_cmd->bss_info.ap_isolate = params->isolate;

    set_bss_cmd->valid_fields = NRF_WIFI_CMD_SET_BSS_HT_OPMODE_VALID |
                    ...
                    NRF_WIFI_CMD_SET_BSS_AP_ISOLATE_VALID;
     {
         ...
         u8 len = 0;
         for (i = 0;
              i < NRF_WIFI_BASIC_MAX_SUPP_RATES && params->basic_rates[i] >=0;
              i++) {
             set_bss_cmd->bss_info.basic_rates[len++] = params->basic_rates[i] / 5;
         }
		 
         set_bss_cmd->bss_info.num_basic_rates = len;
    }

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_BSS command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL,
                     set_bss_cmd,
                     sizeof(*set_bss_cmd),
                     -1);
    ...
    //TODO: Umac to provide set_multicast_to_unicast command for
    // 1. multicast_to_unicastf


    if (beacon_set && params->freq && params->freq->bandwidth != link->bandwidth) {
        wpa_printf(MSG_DEBUG,
               "nl80211: Update BSS %s bandwidth: %d -> %d",
               bss->ifname, link->bandwidth,
               params->freq->bandwidth);
		wpa_printf(MSG_DEBUG, "nl80211: %s, %d\n", __func__, __LINE__);
...
static int nrf_wifi_cfg80211_set_wiphy_params(struct wpa_driver_nl80211_data *drv,
                          struct  nrf_wifi_umac_set_wiphy_info *info)
{
    struct nrf_wifi_umac_cmd_set_wiphy *set_wiphy_cmd = NULL;
    int ret;

    set_wiphy_cmd = os_zalloc(sizeof(*set_wiphy_cmd));
	if (!set_wiphy_cmd) {
                wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
                goto out;
    }
    ...
    set_wiphy_cmd->umac_hdr.ids.wdev_id = 0;//TODO: Hardcoded
    set_wiphy_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    os_memcpy(&set_wiphy_cmd->info,
                info,
                sizeof(set_wiphy_cmd->info));

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_WIPHY command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, set_wiphy_cmd,
            sizeof(*set_wiphy_cmd), -1);

out:
    if (set_wiphy_cmd)
        os_free(set_wiphy_cmd);
			       ...
			       struct hostapd_freq_params *freq, int set_chan)
{
	...
	int ret;
#ifdef CFG_INTERFACE
    //struct nrf_wifi_umac_cmd_set_channel *set_chn_cmd = NULL;
    struct nrf_wifi_umac_cmd_set_wiphy *set_wiphy_cmd = NULL;
#endif
...
#ifdef CFG_INTERFACE
    wpa_printf(MSG_DEBUG, "nl80211: %s, %d, set_chan = %d\n", __func__, __LINE__, set_chan);
   if (set_chan) {
/*
        wpa_printf(MSG_DEBUG, "nl80211: %s, %d\n", __func__, __LINE__);
            set_chn_cmd = os_zalloc(sizeof(*set_chn_cmd));
        if (!set_chn_cmd) {
            wpa_printf(MSG_ERROR, "nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
            return -1;
        }
        set_chn_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_SET_CHANNEL;
        set_chn_cmd->umac_hdr.ids.wdev_id = 0;//TODO: hardcoded
        set_chn_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        if (freq->freq) {
            set_chn_cmd->freq_params.frequency = freq->freq;
            set_chn_cmd->freq_params.valid_fields = NRF_WIFI_SET_FREQ_PARAMS_FREQ_VALID;
            //TODO: set valid flag
            //set_chn_cmd->valid_fields |=
        }

        set_chn_cmd->freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CHANNEL_WIDTH_VALID;
        set_chn_cmd->freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_20_NOHT;
        if (freq->bandwidth) {
            set_chn_cmd->freq_params.channel_width = freq->bandwidth;
        }

        if (freq->center_freq1) {
            set_chn_cmd->freq_params.frequency = freq->bandwidth;
            set_chn_cmd->freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CENTER_FREQ1_VALID;
        }
		 if (freq->center_freq2) {
            set_chn_cmd->freq_params.frequency = freq->bandwidth;
            set_chn_cmd->freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CENTER_FREQ2_VALID;
        }
        set_chn_cmd->freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CHANNEL_TYPE_VALID;

        set_chn_cmd->freq_params.channel_type = NRF_WIFI_CHAN_NO_HT;

        if (freq->ht_enabled) {
            set_chn_cmd->freq_params.channel_type = NRF_WIFI_CHAN_HT20;
            set_chn_cmd->freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_20;
        }

        if (freq->sec_channel_offset == -1) {
            set_chn_cmd->freq_params.channel_type = NRF_WIFI_CHAN_HT40MINUS;
            set_chn_cmd->freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_40;
        }
        if (freq->sec_channel_offset == 1) {
            set_chn_cmd->freq_params.channel_type = NRF_WIFI_CHAN_HT40PLUS;
                    set_chn_cmd->freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_40;
        }

        wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_CHANNEL command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, set_chn_cmd,
                sizeof(*set_chn_cmd), -1);
*/
	} else {
        wpa_printf(MSG_DEBUG, "nl80211: %s, %d\n", __func__, __LINE__);
        ...
        if (!set_wiphy_cmd) {
            ...
            set_wiphy_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_SET_WIPHY;
            set_wiphy_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
            set_wiphy_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        if (freq->freq) {
            set_wiphy_cmd->info.freq_params.frequency = freq->freq;
            set_wiphy_cmd->info.freq_params.valid_fields = NRF_WIFI_SET_FREQ_PARAMS_FREQ_VALID;
        }

        set_wiphy_cmd->info.freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CHANNEL_WIDTH_VALID;
        set_wiphy_cmd->info.freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_20_NOHT;
        if (freq->bandwidth) {
            set_wiphy_cmd->info.freq_params.channel_width = freq->bandwidth;
        }

        if (freq->center_freq1) {
            set_wiphy_cmd->info.freq_params.frequency = freq->bandwidth;
            set_wiphy_cmd->info.freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CENTER_FREQ1_VALID;
        }
        if (freq->center_freq2) {
            set_wiphy_cmd->info.freq_params.frequency = freq->bandwidth;
            set_wiphy_cmd->info.freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CENTER_FREQ2_VALID;
        }
		set_wiphy_cmd->info.freq_params.valid_fields |= NRF_WIFI_SET_FREQ_PARAMS_CHANNEL_TYPE_VALID;

        set_wiphy_cmd->info.freq_params.channel_type = NRF_WIFI_CHAN_NO_HT;

        if (freq->ht_enabled) {
            set_wiphy_cmd->info.freq_params.channel_type = NRF_WIFI_CHAN_HT20;
            set_wiphy_cmd->info.freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_20;
        }

        if (freq->sec_channel_offset == -1) {
            set_wiphy_cmd->info.freq_params.channel_type = NRF_WIFI_CHAN_HT40MINUS;
            set_wiphy_cmd->info.freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_40;
        }
        if (freq->sec_channel_offset == 1) {
            set_wiphy_cmd->info.freq_params.channel_type = NRF_WIFI_CHAN_HT40PLUS;
            set_wiphy_cmd->info.freq_params.channel_width = NRF_WIFI_CHAN_WIDTH_40;
        }

        set_wiphy_cmd->valid_fields |= NRF_WIFI_CMD_SET_WIPHY_FREQ_PARAMS_VALID;
...
#ifdef CFG_INTERFACE
    if (params->set) {
        chg_sta_cmd = os_zalloc(sizeof(*chg_sta_cmd));
        if (!chg_sta_cmd) {
            wpa_printf(MSG_ERROR, "nl80211: %s: Unable to allocate memory. %d\n", __func__, __LINE__);
            return -ENOBUFS;
        }
        chg_sta_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_SET_STATION;
        chg_sta_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hardcoded
        chg_sta_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        os_memcpy(chg_sta_cmd->info.mac_addr,
              params->addr,
              ETH_ALEN);

        if (FULL_AP_CLIENT_STATE_SUPP(drv->capa.flags) &&
            (params->flags & WPA_STA_ASSOCIATED) || (params->flags & WPA_STA_TDLS_PEER)) {
            ...
            if (params->supp_rates_len > 0) {
                os_memcpy(chg_sta_cmd->info.supp_rates.rates,
                      params->supp_rates,
                      params->supp_rates_len);
                chg_sta_cmd->info.supp_rates.nrf_wifi_num_rates = params->supp_rates_len;
                chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_SUPP_RATES_VALID;
            }
            chg_sta_cmd->info.sta_capability = params->capability;
			chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_CAPABILITY_VALID;
            if (params->ht_capabilities) {
                wpa_hexdump(MSG_DEBUG, "  * ht_capabilities",
                        (u8 *) params->ht_capabilities,
                        sizeof(*params->ht_capabilities));
                os_memcpy(chg_sta_cmd->info.ht_capability,
                      params->ht_capabilities,
                      sizeof(*params->ht_capabilities));
                chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_HT_CAPABILITY_VALID;
            }
            ...
            if (params->vht_capabilities) {
                ...
                os_memcpy(chg_sta_cmd->info.vht_capability,
                      params->vht_capabilities,
                      sizeof(*params->vht_capabilities));
                chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_VHT_CAPABILITY_VALID;
            }
            ...
            if (params->ext_capab) {
				wpa_hexdump(MSG_DEBUG, "  * ext_capab",
						...
						os_memcpy(chg_sta_cmd->info.ext_capability.ext_capability,
                        params->ext_capab,
            	        params->ext_capab_len);

                chg_sta_cmd->info.ext_capability.ext_capability_len = params->ext_capab_len;
                chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_EXT_CAPABILITY_VALID;
            }
            //TODO: for p2p
        /*  if (is_ap_interface(drv->nlmode) &&
        ...
        */
        ...
        if (params->aid && (params->flags & WPA_STA_TDLS_PEER)) {
            wpa_printf(MSG_DEBUG, "  * peer_aid=%u", params->aid);
            chg_sta_cmd->info.aid = params->aid;
            chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_AID_VALID;
            ...
            chg_sta_cmd->info.nrf_wifi_listen_interval = params->listen_interval;
            chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_LISTEN_INTERVAL_VALID;
        }
	if (params->vht_opmode_enabled) {
                        wpa_printf(MSG_DEBUG, "  * opmode=%u", params->vht_opmode);
                        chg_sta_cmd->info.opmode_notif = params->vht_opmode;
                        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_OPMODE_NOTIF_VALID;
                }

                if (params->supp_channels) {
                        wpa_hexdump(MSG_DEBUG, "  * supported channels",
                                    params->supp_channels, params->supp_channels_len);
                        os_memcpy(add_sta_cmd->info.supported_channels.supported_channels,
                                  params->supp_channels,
                                  params->supp_channels_len);
                        chg_sta_cmd->info.supported_channels.supported_channels_len = params->supp_channels_len;
                        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_SUPPORTED_CHANNELS_VALID;
                }
		if (params->supp_oper_classes) {
                        ...
                        os_memcpy(add_sta_cmd->info.supported_oper_classes.supported_oper_classes,
                                  params->supp_oper_classes,
                                  params->supp_oper_classes_len);
                        chg_sta_cmd->info.supported_oper_classes.supported_oper_classes_len = params->supp_oper_classes_len;
                        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_SUPPORTED_OPER_CLASSES_VALID;
                }

                if(params->flags & WPA_STA_WMM) {
                        wpa_printf(MSG_DEBUG, "  * qosinfo=0x%x", params->qosinfo);
                        chg_sta_cmd->info.wme_uapsd_queues = params->qosinfo & WMM_QOSINFO_STA_AC_MASK;
                        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_WME_UAPSD_QUEUES_VALID;

                        chg_sta_cmd->info.wme_max_sp = (params->qosinfo >> WMM_QOSINFO_STA_SP_SHIFT) & WMM_QOSINFO_STA_SP_MASK;
                        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_WME_MAX_SP_VALID;
                }

        } else {
                add_sta_cmd = os_zalloc(sizeof(*add_sta_cmd));
                if (!add_sta_cmd) {
                        wpa_printf(MSG_ERROR, "nl80211: %s: Unable to allocate memory. %d\n", __func__, __LINE__);
                        return -ENOBUFS;
                }
                add_sta_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_NEW_STATION;
                add_sta_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hardcoded
                add_sta_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

                os_memcpy(add_sta_cmd->info.mac_addr,
                          params->addr,
                          ETH_ALEN);
                ...
                wpa_printf(MSG_DEBUG, "  * capability=0x%x",
                           params->capability);
                if (params->supp_rates_len > 0) {
                        os_memcpy(add_sta_cmd->info.supp_rates.rates,
                                  params->supp_rates,
                                  params->supp_rates_len);
                        add_sta_cmd->info.supp_rates.nrf_wifi_num_rates = params->supp_rates_len;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_SUPP_RATES_VALID;
                }
                add_sta_cmd->info.sta_capability = params->capability;
                add_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_CAPABILITY_VALID;

		if (params->ht_capabilities) {
                        wpa_hexdump(MSG_DEBUG, "  * ht_capabilities",
                                    (u8 *) params->ht_capabilities,
                                    sizeof(*params->ht_capabilities));
                        os_memcpy(add_sta_cmd->info.ht_capability,
                                  params->ht_capabilities,
                                  sizeof(*params->ht_capabilities));
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_HT_CAPABILITY_VALID;
                }
                ...
                if (params->vht_capabilities) {
                        wpa_hexdump(MSG_DEBUG, "  * vht_capabilities",
                                    (u8 *) params->vht_capabilities,
                                    sizeof(*params->vht_capabilities));
                        os_memcpy(add_sta_cmd->info.vht_capability,
                                  params->vht_capabilities,
                                  sizeof(*params->vht_capabilities));
		 ...
		 if (params->ext_capab) {
                        wpa_hexdump(MSG_DEBUG, "  * ext_capab",
                                    params->ext_capab,
                                    params->ext_capab_len);
                        os_memcpy(add_sta_cmd->info.ext_capability.ext_capability,
                                  params->ext_capab,
                                  params->ext_capab_len);

                        add_sta_cmd->info.ext_capability.ext_capability_len = params->ext_capab_len;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_EXT_CAPABILITY_VALID;
                }
                //TODO: for p2p
        /*      if (is_ap_interface(drv->nlmode) &&
                        nla_put_u8(msg, NL80211_ATTR_STA_SUPPORT_P2P_PS,
        ...
        */
		...
		if (params->aid) {
                        wpa_printf(MSG_DEBUG, "  * aid=%u", params->aid);
                        add_sta_cmd->info.aid = params->aid;
                        ...
                        wpa_printf(MSG_DEBUG, "  * aid=1 (%s workaround)",
                                   ...
                                   "TDLS" : "UNASSOC_STA");
                        add_sta_cmd->info.aid = 1;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_AID_VALID;
                }
                wpa_printf(MSG_DEBUG, "  * listen_interval=%u",
                           params->listen_interval);
                add_sta_cmd->info.nrf_wifi_listen_interval = params->listen_interval;
                add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_LISTEN_INTERVAL_VALID;

                if (params->vht_opmode_enabled) {
                        wpa_printf(MSG_DEBUG, "  * opmode=%u", params->vht_opmode);
                        add_sta_cmd->info.opmode_notif = params->vht_opmode;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_OPMODE_NOTIF_VALID;
                }
		...
		if (params->supp_channels) {
                        ...
                        os_memcpy(add_sta_cmd->info.supported_channels.supported_channels,
                                  params->supp_channels,
                                  params->supp_channels_len);
                        add_sta_cmd->info.supported_channels.supported_channels_len = params->supp_channels_len;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_SUPPORTED_CHANNELS_VALID;
                }
                ...
                if (params->supp_oper_classes) {
                        ...
                        os_memcpy(add_sta_cmd->info.supported_oper_classes.supported_oper_classes,
                                  params->supp_oper_classes,
                                  params->supp_oper_classes_len);
                        add_sta_cmd->info.supported_oper_classes.supported_oper_classes_len = params->supp_oper_classes_len;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_SUPPORTED_OPER_CLASSES_VALID;
                }

                if(params->flags & WPA_STA_WMM) {
                        wpa_printf(MSG_DEBUG, "  * qosinfo=0x%x", params->qosinfo);
                        add_sta_cmd->info.wme_uapsd_queues = params->qosinfo & WMM_QOSINFO_STA_AC_MASK;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_STA_WME_UAPSD_QUEUES_VALID;

                        add_sta_cmd->info.wme_max_sp = (params->qosinfo >> WMM_QOSINFO_STA_SP_SHIFT) & WMM_QOSINFO_STA_SP_MASK;
                        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_STA_WME_MAX_SP_VALID;
                }
        }
...
#ifdef CFG_INTERFACE
    if (params->set) {
        chg_sta_cmd->info.sta_flags2.nrf_wifi_mask = upd.mask;
        chg_sta_cmd->info.sta_flags2.nrf_wifi_set = upd.set;
        chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_FLAGS2_VALID;

        wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_STATION command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, chg_sta_cmd,
                sizeof(*chg_sta_cmd), -1);
        os_free(chg_sta_cmd);
    } else {
        add_sta_cmd->info.sta_flags2.nrf_wifi_mask = upd.mask;
        add_sta_cmd->info.sta_flags2.nrf_wifi_set = upd.set;
        add_sta_cmd->valid_fields |= NRF_WIFI_CMD_NEW_STATION_STA_FLAGS2_VALID;

        wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_NEW_STATION command\n", __func__);
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, add_sta_cmd,
                sizeof(*add_sta_cmd), -1);
        os_free(add_sta_cmd);
    }
					 ...
					 int deauth, u16 reason_code)
{
    ...
    del_sta_cmd = os_zalloc(sizeof(*del_sta_cmd));
    if (!del_sta_cmd) {
        wpa_printf(MSG_ERROR,"nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
        return -ENOBUFS;
    }
    ...
    del_sta_cmd->umac_hdr.ids.wdev_id = 0; //TODO: HardCoded
    del_sta_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    os_memcpy(del_sta_cmd->info.mac_addr,
          addr,
          ETH_ALEN);
    del_sta_cmd->valid_fields |= NRF_WIFI_CMD_DEL_STATION_MAC_ADDR_VALID;

    if (deauth == 0)
        del_sta_cmd->info.mgmt_subtype = WLAN_FC_STYPE_DISASSOC;
    if (deauth == 1)
        del_sta_cmd->info.mgmt_subtype = WLAN_FC_STYPE_DEAUTH;

    del_sta_cmd->valid_fields |= NRF_WIFI_CMD_DEL_STATION_MGMT_SUBTYPE_VALID;
        del_sta_cmd->info.reason_code = reason_code;
    del_sta_cmd->valid_fields |= NRF_WIFI_CMD_DEL_STATION_REASON_CODE_VALID;

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_DEL_STATION command\n", __func__);
    ret = send_and_recv_msgs_cfg(bss->drv, NULL, NULL, del_sta_cmd,
            sizeof(*del_sta_cmd), -1);
    if (ret < 0)
        wpa_printf(MSG_ERROR, "nl80211: %s: send_and_recv_msgs_cfg  failed\n", __func__);
...
#ifdef CFG_INTERFACE
int nrf_wifi_cfg80211_del_vif(struct wpa_driver_nl80211_data *drv,
                  int nrf_wifi_vif_idx)
{
        struct nrf_wifi_rpu_ctx_lnx *rpu_ctx_lnx = NULL;
        struct nrf_wifi_umac_cmd_del_vif *del_vif_cmd = NULL;
        int ret = -1;

    del_vif_cmd = os_zalloc(sizeof(*del_vif_cmd));
        if (!del_vif_cmd) {
            wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory for cmd\n", __func__);
        return -ENOBUFS;
        }

    del_vif_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_DEL_INTERFACE;
        del_vif_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
        del_vif_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_DEL_INTERFACE command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, del_vif_cmd,
            sizeof(*del_vif_cmd), -1);
...
#ifdef CFG_INTERFACE
      ret = nrf_wifi_cfg80211_del_vif(drv, ifidx);
      if (ret == 0)
          return;
#else
...
#ifdef CFG_INTERFACE
struct wdev_info_cfg {
	u64 wdev_id;
	int wdev_id_set;
	u8 macaddr[ETH_ALEN];
};
struct cfg_wdev_info_arg {
      struct wdev_info_cfg *data;
      int result_finished;
};

static int nl80211_wdev_handler_cfg(void *arg, void *msg,
                  int len, int next_avail_data_not_used)
{
      struct nrf_wifi_interface_info *intf_info = (struct nrf_wifi_interface_info *)msg;
      struct cfg_wdev_info_arg *_arg = arg;
      struct wdev_info_cfg *wi = _arg->data;
				     ...
				     void *arg)
{
      ...
      struct cfg_wdev_info_arg wdev_arg;
#else
	struct nl_msg *msg;
#endif
	int ifidx;
	...
	wpa_printf(MSG_DEBUG, "nl80211: Create interface iftype %d (%s)",
...
#ifdef CFG_INTERFACE
      add_vif_info = os_zalloc(sizeof(*add_vif_info));
          if(!add_vif_info) {
              wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory for cmd\n", __func__);
                  goto out;
          }
      ...
      os_memcpy(add_vif_info->ifacename, ifname, sizeof(*ifname));

      if (iftype == NL80211_IFTYPE_MONITOR) {
          add_vif_info->mon_flags = NL80211_MNTR_FLAG_COOK_FRAMES;
      } else if (wds) {
          add_vif_info->nrf_wifi_use_4addr = wds;
      }

      add_vif_cmd = os_zalloc(sizeof(*add_vif_cmd));
          ...
          add_vif_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
          add_vif_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

          os_memcpy(&add_vif_cmd->info,
                    add_vif_info,
                    sizeof(add_vif_cmd->info));

          add_vif_cmd->valid_fields |= NRF_WIFI_CMD_NEW_INTERFACE_IFTYPE_VALID;

      if (add_vif_info->ifacename)
              add_vif_cmd->valid_fields |= NRF_WIFI_CMD_NEW_INTERFACE_IFNAME_VALID;
      if (add_vif_info->mac_addr)
              add_vif_cmd->valid_fields |= NRF_WIFI_CMD_NEW_INTERFACE_MAC_ADDR_VALID;

          wdev_arg.data = arg;
      wdev_arg.result_finished = 1;
  #if 0
      if (handler) {
          ret = send_and_recv_msgs_cfg(drv, nl80211_wdev_handler_cfg, wdev_arg, add_vif_cmd, sizeof(*add_vif_cmd));
          do {
                     os_sleep(0, 10);
          } while(wdev_arg.result_finished);
      } else
  #endif
      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_NEW_INTERFACE command\n", __func__);
      ret = send_and_recv_msgs_cfg(drv, NULL, NULL, add_vif_cmd,
              sizeof(*add_vif_cmd), -1);
  out:
      if (add_vif_info)
          os_free(add_vif_info);
					    ...
					    unsigned int flags_and)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Set STA flags - ifname=%s addr=" MACSTR
      ...
      chg_sta_cmd->info.sta_flags2.nrf_wifi_mask = sta_flags_nl80211(flags_or | ~flags_and);
      chg_sta_cmd->info.sta_flags2.nrf_wifi_set = sta_flags_nl80211(flags_or);
      chg_sta_cmd->valid_fields |= NRF_WIFI_CMD_SET_STATION_STA_FLAGS2_VALID;

      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_STATION command\n", __func__);
      ret = send_and_recv_msgs_cfg(bss->drv, NULL, NULL, chg_sta_cmd,
              sizeof(*chg_sta_cmd), -1);
      os_free(chg_sta_cmd);
...
#ifdef CFG_INTERFACE
  static int nl80211_ht_vht_overrides_cfg(struct nrf_wifi_connect_common_info *connect_common_info,
                      struct wpa_driver_associate_params *params)
  {
      if (params->disable_ht) {
          connect_common_info->ht_vht_capabilities.nrf_wifi_flags |= NL80211_ATTR_DISABLE_HT;
      }
      if (params->htcaps && params->htcaps_mask) {
          ...
          os_memcpy(connect_common_info->ht_vht_capabilities.ht_capability,
                     params->htcaps, sz);
          os_memcpy(connect_common_info->ht_vht_capabilities.ht_capability_mask,
                     params->htcaps_mask, sz);
      }
      ...
      if (params->disable_vht) {
          ...
          connect_common_info->ht_vht_capabilities.nrf_wifi_flags |= NL80211_ATTR_DISABLE_VHT;
      }
...
#ifdef CFG_INTERFACE
          os_memcpy(connect_common_info->ht_vht_capabilities.vht_capability,
                     params->vhtcaps, sz);
          os_memcpy(connect_common_info->ht_vht_capabilities.vht_capability_mask,
                     params->vhtcaps_mask, sz);
 #endif
      }
...
#ifdef CFG_INTERFACE
  static int nl80211_connect_common_cfg(struct wpa_driver_nl80211_data *drv,
                    struct wpa_driver_associate_params *params,
                    struct nrf_wifi_connect_common_info *connect_common_info)
  {
      connect_common_info->nrf_wifi_flags |= NL80211_ATTR_IFACE_SOCKET_OWNER;

      if (params->bssid) {
          wpa_printf(MSG_DEBUG, "  * bssid=" MACSTR,
                 MAC2STR(params->bssid));
          os_memcpy(connect_common_info->mac_addr, params->bssid, ETH_ALEN);
      }

      if (params->bssid_hint) {
          wpa_printf(MSG_DEBUG, "  * bssid_hint=" MACSTR,
                 MAC2STR(params->bssid_hint));
          os_memcpy(connect_common_info->mac_addr_hint, params->bssid_hint, ETH_ALEN);
      }

      if (params->freq.freq) {
          wpa_printf(MSG_DEBUG, "  * freq=%d", params->freq.freq);

          connect_common_info->frequency = params->freq.freq;
      ...
      if (params->freq_hint) {
          ...
          connect_common_info->freq_hint = params->freq_hint;
      }
	
	  if (params->bg_scan_period >= 0) {
          wpa_printf(MSG_DEBUG, "  * bg scan period=%d",
                 params->bg_scan_period);
          connect_common_info->bg_scan_period = params->bg_scan_period;
      }

      if (params->ssid) {
          wpa_printf(MSG_DEBUG, "  * SSID=%s",
                 wpa_ssid_txt(params->ssid, params->ssid_len));
          connect_common_info->ssid.nrf_wifi_ssid_len = params->ssid_len;
          os_memcpy(connect_common_info->ssid.nrf_wifi_ssid, params->ssid, params->ssid_len);
      ...
      if (params->wpa_ie) {
          connect_common_info->wpa_ie.ie_len = params->wpa_ie_len;
          os_memcpy(connect_common_info->wpa_ie.ie, params->wpa_ie, params->wpa_ie_len);
      }
	  if (params->wpa_proto) {
          ...
          connect_common_info->wpa_versions = ver;
      }
      ...
      if (params->pairwise_suite != WPA_CIPHER_NONE) {
          ...
          //connect_common_info->cipher_suite_pairwise = cipher;
      }
      ...
      if (params->group_suite == WPA_CIPHER_GTK_NOT_USED &&
           ...
           */
          wpa_printf(MSG_DEBUG, "  * skip group cipher configuration for GTK_NOT_USED due to missing driver support advertisem  ent");
	  } else if (params->group_suite != WPA_CIPHER_NONE) {
          u32 cipher = wpa_cipher_to_cipher_suite(params->group_suite);
          wpa_printf(MSG_DEBUG, "  * group=0x%x", cipher);
          connect_common_info->cipher_suite_group = cipher;
      }
      ...
      if (params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X ||
          ...
          //connect_common_info->akm_suit = mgmt;
      }
//  if (params->req_handshake_offload &&
  //      (drv->capa.flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_8021X)) {
  //          wpa_printf(MSG_DEBUG, "  * WANT_1X_4WAY_HS");
  //      connect_common_info->flags |= NL80211_ATTR_WANT_1X_4WAY_HS;
  //  }
  ...
  //  connect_common_info->flags |= NL80211_ATTR_CONTROL_PORT;
      connect_common_info->control_port = 1;
      ...
      if (params->key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_NO_WPA &&
          ...
          connect_common_info->control_port_ether_type = ETH_P_PAE;
          connect_common_info->nrf_wifi_flags |= NRF_WIFI_CONNECT_COMMON_INFO_CONTROL_PORT_ETHER_TYPE;
      }
      ...
      if (params->rrm_used) {
          ...
          if ((!((drv_rrm_flags &
              ...
              connect_common_info->nrf_wifi_flags |= NRF_WIFI_CMD_CONNECT_COMMON_INFO_USE_RRM;
      }

      if (nl80211_ht_vht_overrides_cfg(connect_common_info,
                      params) < 0)
          return -1;
      if (params->p2p)
...
#if 0
          connect_common_info->flags |= NL80211_ATTR_PBSS;
#endif
      }
      ...
      if (params->prev_bssid) {
          ...
          os_memcpy(connect_common_info->prev_bssid,
                  params->prev_bssid, ETH_ALEN);
		connect_common_info->nrf_wifi_flags |= NRF_WIFI_CONNECT_COMMON_INFO_PREV_BSSID;
          drv->connect_reassoc = 1;
      }

#ifdef CFG_INTERFACE
      //TODO : We dont need below probably check again with UMAC team
#if 0
...
#ifdef CFG_INTERFACE
int nrf_wifi_cfg80211_assoc(struct wpa_driver_nl80211_data *drv,
          int ifindex, struct nrf_wifi_connect_common_info *assoc_info)
{
      struct nrf_wifi_umac_cmd_assoc *assoc_cmd = NULL;
      int ret = -1;
      struct nrf_wifi_connect_common_info *connect_common_info = NULL;

      assoc_cmd = os_zalloc(sizeof(*assoc_cmd));
      ...
      connect_common_info->valid_fields |=
          ...
          if (assoc_info->ssid.nrf_wifi_ssid_len > 0) {
                  os_memcpy(&connect_common_info->ssid,
                            &assoc_info->ssid,
	...
	if (assoc_info->nrf_wifi_flags & NRF_WIFI_CONNECT_COMMON_INFO_PREV_BSSID) {
           os_memcpy(&connect_common_info->prev_bssid,
                 &assoc_info->prev_bssid, NRF_WIFI_ETH_ALEN);
	}

	connect_common_info->nrf_wifi_flags = assoc_info->nrf_wifi_flags;
		  wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_ASSOCIATE command\n", __func__);
		  ret = send_and_recv_msgs_cfg(drv, NULL, NULL, assoc_cmd,
          		    sizeof(*assoc_cmd), -1);
out:
          if(assoc_cmd)
                  os_free(assoc_cmd);
	...
	void *priv, struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Associate (ifindex=%d)",
		   drv->ifindex);
#ifdef CFG_INTERFACE
      connect_common_info = os_zalloc(sizeof(*connect_common_info));

          if(!connect_common_info) {
                  wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
                  goto fail;
          }

      ret = nl80211_connect_common_cfg(drv, params, connect_common_info);
#else
...
#ifdef CFG_INTERFACE
      if (params->mgmt_frame_protection == MGMT_FRAME_PROTECTION_REQUIRED)
          connect_common_info->use_mfp = NRF_WIFI_MFP_REQUIRED;
#else
	if (params->mgmt_frame_protection == MGMT_FRAME_PROTECTION_REQUIRED &&
	    nla_put_u32(msg, NL80211_ATTR_USE_MFP, NL80211_MFP_REQUIRED))
		goto fail;
#endif
#ifdef CFG_INTERFACE
      ret = nrf_wifi_cfg80211_assoc(drv, drv->ifindex, connect_common_info);
#else
...
#ifdef CFG_INTERFACE
  struct get_setintf_res {
          u8 *res;
          int result_finished;
  };

  static int get_setintf_res_cfg(void *arg, void *msg,
                                   int len, int next_avail_data_not_used)
    {
            struct nrf_wifi_umac_event_set_interface *info = (struct nrf_wifi_umac_event_set_interface *)msg;
            struct get_setintf_res *_arg = arg;

        *(_arg->res) = info->return_value;
			    ...
			    int ifindex, enum nl80211_iftype mode)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Set mode ifindex %d iftype %d (%s)",
...
#ifdef CFG_INTERFACE
          chg_vif_cmd = os_zalloc(sizeof(*chg_vif_cmd));
          if (!chg_vif_cmd) {
                  wpa_printf(MSG_ERROR, "nl80211: %s: Unable to allocate memory %d\n", __func__, __LINE__);
                  return -ENOBUFS;
          }
          ...
          chg_vif_cmd->umac_hdr.ids.wdev_id = 0;//TODO: hardcoded
          ...
          chg_vif_cmd->info.iftype = mode;
          arg.res = &res;

      ret = send_and_recv_msgs_cfg(drv, get_setintf_res_cfg, &arg, chg_vif_cmd,
              sizeof(*chg_vif_cmd), NRF_WIFI_UMAC_EVENT_SET_INTERFACE);
      if (ret < 0)
          wpa_printf(MSG_DEBUG, "nl80211: %s: send_and_recv_msgs_cfg failed\n", __func__);
      ...
      if (!(*arg.res))
       ...
       wpa_printf(MSG_DEBUG, "nl80211: Failed to set interface %d to mode %d:"
             " %d (%s)", ifindex, mode, res, strerror(-res));
      return res;
#else
...
#ifdef CFG_INTERFACE
struct get_seqnum_arg {
        u8 *seq;
        int result_finished;
};

static int get_key_handler_cfg(void *arg, void *msg,
                               int len, int next_avail_data_not_used)
{
#if 0
        struct nrf_wifi_umac_event_get_key *info = (struct nrf_wifi_umac_event_get_key *)msg;
        struct get_seqnum_arg *_arg = arg;


        u8 *seq  = _arg->seq;

        wpa_printf(MSG_DEBUG, "%s: Enter %d\n", __func__, __LINE__);
        /*
         * TODO: validate the key index and mac address!
         * Otherwise, there's a race condition as soon as
         * the kernel starts sending key notifications.
        ...
        if (info->key_info.valid_fields & NRF_WIFI_SEQ_VALID) {
                memcpy(seq,
                       info->key_info.seq.nrf_wifi_seq,
                       min_int(info->key_info.seq.nrf_wifi_seq_len, 6));
        }
        _arg->result_finished = 0;
#endif
        return NL_SKIP;
			   ...
			   int idx, int link_id, u8 *seq)
{
	...
	struct nl_msg *msg;
#ifdef CFG_INTERFACE
      struct nrf_wifi_umac_cmd_key *get_key_cmd;
      struct get_seqnum_arg arg;
      int ret;

          get_key_cmd = os_zalloc(sizeof(*get_key_cmd));
          if (!get_key_cmd) {
                  wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
                  return -ENOBUFS;
          }

          get_key_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_KEY;
          get_key_cmd->umac_hdr.ids.wdev_id = 0;//TODO: hardcoded
          get_key_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

          if (addr) {
                  os_memcpy(get_key_cmd->mac_addr,
                            addr,
                            ETH_ALEN);
                  get_key_cmd->valid_fields |= NRF_WIFI_CMD_KEY_MAC_ADDR_VALID;
      }

      //get_key_cmd->key_idx = idx;
      //get_key_cmd->valid_fields |= NRF_WIFI_CMD_GET_KEY_KEY_IDX_VALID;


          arg.seq = seq;
          arg.result_finished = 1;

      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_GET_KEY command\n", __func__);
      ret = send_and_recv_msgs_cfg(bss->drv, get_key_handler_cfg, &arg, get_key_cmd,
              sizeof(*get_key_cmd), NRF_WIFI_UMAC_EVENT_GET_KEY);
...
static int i802_set_rts(void *priv, int rts)
{
	...
	if (rts >= 2347 || rts == -1)
      ...
      set_wiphy_cmd->info.rts_threshold = val;
      set_wiphy_cmd->valid_fields = NRF_WIFI_CMD_SET_WIPHY_RTS_THRESHOLD_VALID;

      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_WIPHY command\n", __func__);
      ret = send_and_recv_msgs_cfg(drv, NULL, NULL, set_wiphy_cmd, sizeof(*set_wiphy_cmd), -1);
...
static int i802_set_frag(void *priv, int frag)
{
	...
	if (frag >= 2346 || frag == -1)
		...
		val = frag;
          ...
          set_wiphy_cmd->info.frag_threshold = val;
          set_wiphy_cmd->valid_fields = NRF_WIFI_CMD_SET_WIPHY_FRAG_THRESHOLD_VALID;
...
#ifdef CFG_INTERFACE
struct cfg_station_signal_info_arg {
/*        struct wpa_signal_info *data;        // 2.10 variant */
       struct hostap_sta_driver_data *data;
        int result_finished;
};

static int get_link_signal_handler_cfg(void *arg, void *msg,
                               int len, int next_avail_data_not_used)
{
        struct nrf_wifi_umac_event_new_station *info = (struct nrf_wifi_umac_event_new_station *)msg;
        struct cfg_station_signal_info_arg *_arg = arg;
        struct hostap_sta_driver_data *sig_change = _arg->data;

	if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_SIGNAL_VALID)
	        sig_change->signal = info->sta_info.signal;

	if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_SIGNAL_AVG_VALID)
		sig_change->avg_signal = info->sta_info.signal_avg;
	else
		sig_change->avg_signal = 0;

	if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_RX_BEACON_SIGNAL_AVG_VALID)
		sig_change->avg_beacon_signal = info->sta_info.rx_beacon_signal_avg;
	else
		sig_change->avg_beacon_signal = 0;

	sig_change->current_tx_rate = 0;
	...
	if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_BITRATE_VALID) {
		if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
			sig_change->current_tx_rate = info->sta_info.tx_bitrate.bitrate * 100;
	}
			    ...
			    struct hostap_sta_driver_data *data)
{
...
#ifdef CFG_INTERFACE
	struct cfg_station_signal_info_arg arg;
	struct nrf_wifi_umac_cmd_get_sta *get_sta_cmd = NULL;
	int ret;

	get_sta_cmd = os_zalloc(sizeof(*get_sta_cmd));
	...
	get_sta_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_GET_STATION;
	get_sta_cmd->umac_hdr.ids.wdev_id = 0;//TODO: hardcoded
	get_sta_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

	os_memcpy(get_sta_cmd->info.mac_addr,
		  drv->bssid,
		  ETH_ALEN);
	...
	wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_GET_STATION command\n", __func__);

	ret = send_and_recv_msgs_cfg(drv, get_link_signal_handler_cfg, &arg, get_sta_cmd,
				     sizeof(*get_sta_cmd), NRF_WIFI_UMAC_EVENT_GET_STATION);
...
#ifdef CFG_INTERFACE
  struct cfg_station_info_arg {
      struct hostap_sta_driver_data *data;
      int result_finished;
  };

  static int get_sta_handler_cfg(void *arg, void *msg,
                     int len, int next_avail_data_not_used)
  {
          struct nrf_wifi_umac_event_new_station *info = (struct nrf_wifi_umac_event_new_station *)msg;
          struct cfg_station_info_arg *_arg = arg;
          struct hostap_sta_driver_data *data = _arg->data;
      ...
      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_INACTIVE_TIME_VALID)
          data->inactive_msec = info->sta_info.inactive_time;
      /* For backwards compatibility, fetch the 32-bit counters first. */
      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_RX_BYTES_VALID)
          data->rx_bytes = info->sta_info.rx_bytes;

      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_BYTES_VALID)
          data->tx_bytes = info->sta_info.tx_bytes;

      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_RX_PACKETS_VALID)
          data->rx_packets = info->sta_info.rx_packets;
	  if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_PACKETS_VALID)
          data->tx_packets = info->sta_info.tx_packets;


      /*
      TODO: Didnt get from UMAC ( may be due to older version)
      if (stats[NL80211_STA_INFO_RX_DURATION])
          data->rx_airtime =
              nla_get_u64(stats[NL80211_STA_INFO_RX_DURATION]);
      if (stats[NL80211_STA_INFO_TX_DURATION])
      ...
      */
      ...
      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_FAILED_VALID)
          data->tx_retry_failed = info->sta_info.tx_failed;

      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_SIGNAL_AVG_VALID)
          data->signal = info->sta_info.signal_avg;


      if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_SIGNAL_VALID) {
          data->last_ack_rssi = info->sta_info.signal;
          data->flags |= STA_DRV_DATA_LAST_ACK_RSSI;
      }
	
	  if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_BITRATE_VALID) {
          if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
              data->current_tx_rate = info->sta_info.tx_bitrate.bitrate;
          else if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_COMPAT_VALID)
              data->current_tx_rate = info->sta_info.tx_bitrate.bitrate_compat;

          if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_MCS_VALID) {
              data->tx_mcs = info->sta_info.tx_bitrate.nrf_wifi_mcs;
              data->flags |= STA_DRV_DATA_TX_MCS;
          }

          if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_MCS_VALID) {
              data->tx_vhtmcs = info->sta_info.tx_bitrate.vht_mcs;
              data->flags |= STA_DRV_DATA_TX_VHT_MCS;
          }

          if (info->sta_info.tx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_SHORT_GI)
              data->flags |= STA_DRV_DATA_TX_SHORT_GI;

          if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_NSS_VALID) {
              data->tx_vht_nss = info->sta_info.tx_bitrate.vht_nss;
              data->flags |= STA_DRV_DATA_TX_VHT_NSS;
          }
      }
	
	  if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_RX_BITRATE_VALID) {
          if (info->sta_info.rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
              data->current_rx_rate = info->sta_info.rx_bitrate.bitrate;
          else if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_COMPAT_VALID)
              data->current_rx_rate = info->sta_info.rx_bitrate.bitrate_compat;

          if (info->sta_info.rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_MCS_VALID) {
              data->rx_mcs = info->sta_info.rx_bitrate.nrf_wifi_mcs;
              data->flags |= STA_DRV_DATA_RX_MCS;
          }
          if (info->sta_info.rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_MCS_VALID) {
              data->rx_vhtmcs = info->sta_info.rx_bitrate.vht_mcs;
              data->flags |= STA_DRV_DATA_RX_VHT_MCS;
          }
          if (info->sta_info.rx_bitrate.nrf_wifi_flags & NRF_WIFI_RATE_INFO_SHORT_GI)
              data->flags |= STA_DRV_DATA_RX_SHORT_GI;
          if (info->sta_info.rx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VHT_NSS_VALID) {
              data->rx_vht_nss = info->sta_info.rx_bitrate.vht_nss;
              data->flags |= STA_DRV_DATA_RX_VHT_NSS;
          }
      }
			      ...
			      const u8 *addr)
{
	struct nl_msg *msg;
#ifdef CFG_INTERFACE
      struct cfg_station_info_arg arg;
      struct nrf_wifi_umac_cmd_get_sta *get_sta_cmd = NULL;
      int ret;
      get_sta_cmd = os_zalloc(sizeof(*get_sta_cmd));
          if (!get_sta_cmd) {
      ...
      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_GET_STATION command\n", __func__);
      ret = send_and_recv_msgs_cfg(bss->drv, get_sta_handler_cfg, &arg, get_sta_cmd,
              sizeof(*get_sta_cmd), NRF_WIFI_UMAC_EVENT_GET_STATION);
				    ...
				    int link_id)
{
	...
	int res;
      ...
      switch(queue) {
      case 0:
          set_wiphy_cmd->info.txq_params.ac = NL80211_TXQ_Q_VO;
          break;
      case 1:
          set_wiphy_cmd->info.txq_params.ac = NL80211_TXQ_Q_VI;
          break;
      case 2:
          set_wiphy_cmd->info.txq_params.ac = NL80211_TXQ_Q_BE;
          break;
      case 3:
          set_wiphy_cmd->info.txq_params.ac = NL80211_TXQ_Q_BK;
          break;
      }

      set_wiphy_cmd->info.txq_params.txop = ((burst_time * 100 + 16) / 32);
      set_wiphy_cmd->info.txq_params.cwmin = cw_min;
          set_wiphy_cmd->info.txq_params.cwmax = cw_max;
          set_wiphy_cmd->info.txq_params.aifs = aifs;
          set_wiphy_cmd->valid_fields = NRF_WIFI_CMD_SET_WIPHY_TXQ_PARAMS_VALID;
	
      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_WIPHY command\n", __func__);
          res = send_and_recv_msgs_cfg(drv, NULL, NULL, set_wiphy_cmd,
              sizeof(*set_wiphy_cmd), -1);
...
#ifdef CFG_INTERFACE
struct cfg_cookie_info_arg {
          u64 *cookie;
      struct wpa_driver_nl80211_data *drv;
      int result_finished;
};

static int cookie_handler_cfg(void *arg, struct nrf_wifi_umac_event_cookie_rsp *cookie_rsp,
                  int len, int next_avail_data_not_used)
{
          struct cfg_cookie_info_arg *_arg = arg;
      u64 *cookie = _arg->cookie;
      struct cookie_info *cookie_info = NULL;
      struct wpa_driver_nl80211_data *drv = _arg->drv;

      cookie_info = os_zalloc(sizeof(*cookie_info));
          if (!cookie_info)
                  return -ENOMEM;
          ...
          dl_list_add(&drv->global->cookie_list,  &cookie_info->list);

      //if (cookie_rsp->valid_fields & NRF_WIFI_CMD_COOKIE_RSP_COOKIE_VALID)
      *cookie = cookie_rsp->host_cookie;
      _arg->result_finished = 0;
...
#ifdef CFG_INTERFACE
  //TODO: SeT these valid fields before calling this API
int nrf_wifi_cfg80211_mgmt_tx(struct wpa_driver_nl80211_data *drv,
          int ifindex, struct nrf_wifi_umac_mgmt_tx_info *mgmt_tx_info)
{
      struct nrf_wifi_umac_cmd_mgmt_tx *mgmt_tx_cmd = NULL;
          int ret = -1;

      mgmt_tx_cmd = os_zalloc(sizeof(*mgmt_tx_cmd));
          if(!mgmt_tx_cmd) {
                  wpa_printf(MSG_DEBUG, "%s: Unable to allocate memory\n", __func__);
                  return -ENOBUFS;
          }
          ...
          mgmt_tx_cmd->umac_hdr.ids.wdev_id = 0; //TODO: Hard Coded
          mgmt_tx_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

          os_memcpy(&mgmt_tx_cmd->info,
                    mgmt_tx_info,
      ...
      wpa_printf(MSG_DEBUG, "nl80211: %s: Sending NRF_WIFI_UMAC_CMD_FRAME command\n", __func__);
      ret = send_and_recv_msgs_cfg(drv, NULL, NULL, mgmt_tx_cmd,
              sizeof(*mgmt_tx_cmd), -1);
				  ...
				  size_t csa_offs_len, int link_id)
{
    ...
    struct nrf_wifi_umac_cmd_mgmt_tx *mgmt_tx_cmd = NULL;
    struct cfg_cookie_info_arg arg;
    ...
    mgmt_tx_cmd = os_zalloc(sizeof(*mgmt_tx_cmd));
        ...
        mgmt_tx_info = os_zalloc(sizeof(*mgmt_tx_info));
        ...
        if (wait)
                mgmt_tx_info->dur = wait;

        if (buf_len) {
                os_memcpy(mgmt_tx_info->frame.frame,
                       buf,
                       buf_len);
		mgmt_tx_info->frame.frame_len = buf_len;
	}
    if (!no_ack)
        mgmt_tx_info->host_cookie = drv->global->mgmt_tx_cmd_frame_cookie++;

        mgmt_tx_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_FRAME;
        //mgmt_tx_cmd->umac_hdr.ids.wdev_id = ifindex;
        mgmt_tx_cmd->umac_hdr.ids.wdev_id = 0; // Hard Coded
        mgmt_tx_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;
        ...
        mgmt_tx_cmd->info.freq_params.valid_fields |=
                (NRF_WIFI_SET_FREQ_PARAMS_FREQ_VALID |
                 ...
                 NRF_WIFI_SET_FREQ_PARAMS_CENTER_FREQ2_VALID |
                 NRF_WIFI_SET_FREQ_PARAMS_CHANNEL_TYPE_VALID);
    //TODO: usesocket to send the command
    //
    ...
    if (WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_PROBE_RESP) {
	        wpa_printf(MSG_DEBUG, "SENDING PROBE RESP no_ack = %d ", no_ack);
    } else if (WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ASSOC_RESP) {
        wpa_printf(MSG_DEBUG, "SENDING ASSOC RESP no_ack = %d ", no_ack);
    } else if (WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_AUTH) {
        wpa_printf(MSG_DEBUG, "SENDING AUTH RESP no_ack = %d ", no_ack);
    } else if (WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_DEAUTH) {
        wpa_printf(MSG_DEBUG, "SENDING DEAUTH no_ack = %d", no_ack);
    } else {
        wpa_printf(MSG_DEBUG," SENDING OTHER no_ack = %d", no_ack);
    }
    if (!no_ack)
        ret = send_and_recv_msgs_cfg(drv, cookie_handler_cfg, &arg, mgmt_tx_cmd,
                sizeof(*mgmt_tx_cmd), NRF_WIFI_UMAC_EVENT_COOKIE_RESP );
    else
        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, mgmt_tx_cmd,
                sizeof(*mgmt_tx_cmd), -1);
#else
						...
						unsigned int duration)
{
	...
	u64 cookie;
#ifdef CFG_INTERFACE
	roc_cmd = os_zalloc(sizeof(*roc_cmd));
        ...
        if (duration > 0)
                roc_cmd->info.dur = duration;

        roc_cmd->info.nrf_wifi_freq_params.frequency =  freq;

	roc_cmd->info.host_cookie = drv->global->remain_on_channel_cookie++;

        roc_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_REMAIN_ON_CHANNEL;
        roc_cmd->umac_hdr.ids.wdev_id = bss->wdev_id; //CHECK IT
        roc_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;
        
	if (roc_cmd->info.dur != 0)
	...
	ret = send_and_recv_msgs_cfg(drv, cookie_handler_cfg, &arg, roc_cmd,
			sizeof(*roc_cmd), NRF_WIFI_UMAC_EVENT_COOKIE_RESP);
...
static int wpa_driver_nl80211_cancel_remain_on_channel(void *priv)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Cancel remain-on-channel with cookie "
        ...
        cancel_roc_cmd = os_zalloc(sizeof(*cancel_roc_cmd));
        ...
        cancel_roc_cmd->umac_hdr.ids.wdev_id = bss->wdev_id; //Hard coded , CHECK it
        cancel_roc_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;
        cancel_roc_cmd->cookie = drv->remain_on_chan_cookie;
        cancel_roc_cmd->valid_fields |= NRF_WIFI_CMD_CANCEL_ROC_COOKIE_VALID;

        ret = send_and_recv_msgs_cfg(drv, NULL, NULL, cancel_roc_cmd,
			sizeof(*cancel_roc_cmd), -1);
out:
	if (cancel_roc_cmd)
		os_free(cancel_roc_cmd);
#else
...
static int wpa_driver_nl80211_probe_req_report(struct i802_bss *bss, int report)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
#ifdef CFG_INTERFACE
	    wpa_printf(MSG_DEBUG, "Sending Register PROBE_REQ \n");

		 if (nl80211_register_frame(bss, NULL,
		                (WLAN_FC_TYPE_MGMT << 2) |
...
{
#ifdef CFG_INTERFACE
    wpa_printf(MSG_DEBUG, "Returnning as this is not supported\n");
    return 0;
#else
	struct nl_msg *msg;
	struct nlattr *bands, *band;
...
#ifdef CFG_INTERFACE
struct cfg_chanl_width_info_arg {
        struct wpa_signal_info *data;
    int result_finished;
};

static int get_channel_width_cfg(void *arg, void *msg,
                int len, int next_avail_data_not_used)
{
    struct nrf_wifi_interface_info *intf_info = (struct nrf_wifi_interface_info *)msg;
    struct cfg_chanl_width_info_arg *_arg = arg;
    struct wpa_signal_info *sig_change = _arg->data;
    ...
    sig_change->chanwidth = convert2width(chan_def_info->width);
    sig_change->center_frq1 = chan_def_info->center_frequency1;
    sig_change->center_frq2 = chan_def_info->center_frequency2;
				     ...
				     struct wpa_signal_info *sig)
{
    ...
    struct nrf_wifi_cmd_get_interface *get_intf_cmd;
    int ret = -1;
    struct cfg_chanl_width_info_arg arg;
    ...
    ret = send_and_recv_msgs_cfg(drv, get_channel_width_cfg, &arg, get_intf_cmd,
            sizeof(*get_intf_cmd), NRF_WIFI_UMAC_EVENT_NEW_INTERFACE);
...
static int nl80211_signal_poll(void *priv, struct wpa_signal_info *si)
{
	...
	if (res != 0)
		return res;
#ifdef CFG_INTERFACE
    //We don't support command to get Noise
    //so assigning WFA_INVALID_NOISE
    si->current_noise = WPA_INVALID_NOISE;
    si->frequency = drv->assoc_freq;
    return res;
    //return -EOPNOTSUPP;
#else
...
#ifdef CFG_INTERFACE
    global->cfg_netlink = cfg_netlink_init(global);
    dl_list_init(&global->cookie_list);
    dl_list_init(&global->cfg_cbs_list);
    global->remain_on_channel_cookie = 0;
    global->mgmt_tx_cmd_frame_cookie = 0;
#endif
...
static void nl80211_global_deinit(void *priv)
{
    ...
    struct no_cfg_cb_info *cb_info = NULL;
    struct no_cfg_cb_info *cb_tmp = NULL;
#endif
	if (global == NULL)
		return;
...
#ifdef CFG_INTERFACE
    if (global->cfg_netlink)
           cfg_netlink_deinit(global->cfg_netlink);

    if (!dl_list_empty(&global->cookie_list)) {
            dl_list_for_each_safe(cookie_info, tmp, &global->cookie_list,
    ...
    if (!dl_list_empty(&global->cfg_cbs_list)) {
                dl_list_for_each_safe(cb_info, cb_tmp, &global->cfg_cbs_list,
...
{
#ifdef CFG_INTERFACE
    struct nrf_wifi_umac_cmd_set_power_save *set_ps_cmd;
#else
	struct nl_msg *msg;
#endif
	int ret;
#ifdef CFG_INTERFACE
    set_ps_cmd = os_zalloc(sizeof(*set_ps_cmd));
        if(!set_ps_cmd) {
                wpa_printf(MSG_DEBUG,"%s: Unable to allocate memory\n", __func__);
                return -ENOBUFS;
        }
    ...
    set_ps_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_SET_POWER_SAVE;
        set_ps_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
        set_ps_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

    set_ps_cmd->info.ps_state = enabled ? NRF_WIFI_PS_ENABLED : NRF_WIFI_PS_DISABLED;

    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_POWER_SAVE command\n", __func__);
    ret = send_and_recv_msgs_cfg(bss->drv, NULL, NULL, set_ps_cmd,
            sizeof(*set_ps_cmd), -1);
				  ...
				  struct wpa_driver_set_key_params *params)
{
	struct i802_bss *bss = priv;
#ifdef CFG_INTERFACE
    return wpa_driver_nl80211_set_key_cfg(params->ifname, bss, params->alg, params->addr, params->key_idx,
                      params->set_tx, params->seq, params->seq_len, params->key, params->key_len);
#else
...
#endif /* CONFIG_DRIVER_NL80211_QCA */
#ifdef CFG_INTERFACE
    return  wpa_driver_nl80211_scan_cfg(bss, params);
#else
			       ...
			       u8 qos_map_set_len)
{
...
#ifdef CFG_INTERFACE
        set_qos_cmd = os_zalloc(sizeof(*set_qos_cmd));
        if(!set_qos_cmd) {
                wpa_printf(MSG_DEBUG,"%s: Unable to allocate memory\n", __func__);
        return -ENOBUFS;
        }
    ...
    set_qos_cmd->umac_hdr.cmd_evnt = NRF_WIFI_UMAC_CMD_SET_QOS_MAP;
        set_qos_cmd->umac_hdr.ids.wdev_id = 0;//TODO:Hard Coded
        set_qos_cmd->umac_hdr.ids.valid_fields |= NRF_WIFI_INDEX_IDS_WDEV_ID_VALID;

        if (qos_map_set_len) {
                os_memcpy(&set_qos_cmd->info.qos_map_info,
                          qos_map_set,
                          qos_map_set_len);
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_QOS_MAP command\n", __func__);
    ret = send_and_recv_msgs_cfg(drv, NULL, NULL, set_qos_cmd,
            sizeof(*set_qos_cmd), -1);
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.radio_disable = testing_nl80211_radio_disable,
        ...
        .display_scan = nrf_wifi_display_scan,
        .get_display_scan_results = nrf_wifi_get_display_scan_results,
        .retrieve_display_scan_results = nrf_wifi_retrieve_display_scan_results,
#endif
};
