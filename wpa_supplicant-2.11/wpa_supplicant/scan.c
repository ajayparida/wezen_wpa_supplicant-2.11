		...
		while (ssid) {
			if (!wpas_network_disabled(wpa_s, ssid) &&
				...
				params.num_ssids++;
                                ...
                                /* Note: in umac cfg case, scan_ssid is being
                                 * set for connectiontion scan which
                                 * should not be done. we don't support wild
                                 * card scan. This is our special requirement.
                                 * so below two line moved down
                                 * to add other ssid. Max ssids we support is 2.
                                 */
                                if (params.num_ssids + 1 >= max_ssids)
                                        break;
#endif
...
int wpa_supplicant_display_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	return wpa_drv_display_scan(wpa_s);
}

int wpa_supplicant_get_display_scan_results(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
        return wpa_drv_get_display_scan_results(wpa_s, buf, buflen);
}

int wpa_supplicant_retrieve_display_scan_results(struct wpa_supplicant *wpa_s)
{
        return wpa_drv_retrieve_display_scan_results(wpa_s);
}
#endif
#endif
