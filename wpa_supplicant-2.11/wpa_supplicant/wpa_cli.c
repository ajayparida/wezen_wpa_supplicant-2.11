...
// NEWLY ADDED for displayinng all scan result
static int 
wpa_ctrl_command_display_scan_result(struct wpa_ctrl *ctrl, const char *cmd, int print)
{
	char buf[10240];
	size_t len;
	int ret;
...
#ifdef CFG_INTERFACE
static int wpa_cli_cmd_sae_pwe(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
        return wpa_cli_cmd(ctrl, "SAE_PWE", 1, argc, argv);
}

#ifdef BSS_OPTIMIZATION
static int wpa_cli_cmd_display_scan(struct wpa_ctrl *ctrl, int argc,
                                   char *argv[])
{
        return wpa_cli_cmd(ctrl, "DISPLAY_SCAN", 0, argc, argv);
}

static int wpa_cli_cmd_get_display_scan_result(struct wpa_ctrl *ctrl, int argc,
                                   char *argv[])
{
        return wpa_ctrl_command_display_scan_result(ctrl, "DISPLAY_SCAN_RESULTS", 1);
}
#endif
#endif
...
#ifdef CFG_INTERFACE
	{ "sae_pwe", wpa_cli_cmd_sae_pwe, NULL,
          cli_cmd_flag_none,
          "<value> = set sae_pwe parameter" },
#ifdef BSS_OPTIMIZATION
        { "display_scan", wpa_cli_cmd_display_scan, NULL,
          cli_cmd_flag_none,
          "= request new BSS DISPLAY scan" },
        { "get_display_scan_result", wpa_cli_cmd_get_display_scan_result, NULL,
          cli_cmd_flag_none,
          "= get display scan results" },
#endif
#endif
