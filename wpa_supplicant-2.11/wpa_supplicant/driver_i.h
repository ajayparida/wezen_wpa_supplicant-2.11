...
static inline int wpa_drv_display_scan(struct wpa_supplicant *wpa_s)
{
        if (wpa_s->driver->display_scan)
                return wpa_s->driver->display_scan(wpa_s->drv_priv);
        return -1;
}

static inline int wpa_drv_retrieve_display_scan_results(struct wpa_supplicant *wpa_s)
{
        if (wpa_s->driver->retrieve_display_scan_results)
                return wpa_s->driver->retrieve_display_scan_results(wpa_s->drv_priv);
        return -1;
}

static inline int wpa_drv_get_display_scan_results(struct wpa_supplicant *wpa_s, char *buf, size_t len)
{
        if (wpa_s->driver->get_display_scan_results)
                return wpa_s->driver->get_display_scan_results(wpa_s->drv_priv, buf, len);
        return -1;
}
#endif
...
{
	wpa_dbg(wpa_s, MSG_ERROR,"KARUN-------%s-------%d----------", __func__, __LINE__);
