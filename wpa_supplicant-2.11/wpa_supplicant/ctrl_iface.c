					   ...
					   char *cmd)
{
	...
	*pos++ = '\0';
	...
	ssid->bssid_set = !is_zero_ether_addr(bssid);
        ...
        os_memcpy(wpa_s->display_scan_bssid, bssid, ETH_ALEN);
#endif
	return 0;
}
...
#ifdef CFG_INTERFACE
static int wpa_supplicant_ctrl_iface_sae_pwe(
        struct wpa_supplicant *wpa_s, char *cmd)
{
        int sae_pwe = atoi(cmd);
...
static void wpas_ctrl_display_scan(struct wpa_supplicant *wpa_s, char *params,
...
#if 0
		if (wpa_supplicant_display_scan(wpa_s, 0, 0) == -1)
			goto out;
#endif
		wpa_supplicant_display_scan(wpa_s, 0, 0);
		return;
	}
out:
#define WPA_EVENT_SCAN_IN_PROGRESS "CTRL-EVENT-SCAN-IN-PROGRESS "
    wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_SCAN_IN_PROGRESS);
	return;			
}

static int wpas_ctrl_iface_get_display_scan_results(struct wpa_supplicant *wpa_s, char *buf,
                           size_t buflen)
{
        return wpa_supplicant_get_display_scan_results(wpa_s, buf, buflen);
}
#endif
#endif
...
#ifdef CFG_INTERFACE
        } else if (os_strncmp(buf, "SAE_PWE ", 8) == 0) {
		 if (wpa_supplicant_ctrl_iface_sae_pwe(wpa_s, buf + 8))
                        reply_len = -1;
#ifdef BSS_OPTIMIZATION
        } else if (os_strcmp(buf, "DISPLAY_SCAN") == 0) {
                wpas_ctrl_display_scan(wpa_s, NULL, reply, reply_size, &reply_len);
        } else if (os_strcmp(buf, "DISPLAY_SCAN_RESULTS") == 0) {
                reply_len = wpas_ctrl_iface_get_display_scan_results(
                        wpa_s,  reply, reply_size);
#endif
#endif
